...
**23 septembre 2026 — lire d'abord.** Le jeu n'affiche plus de 3D en temps réel : il montre des **sprites cuits depuis les GLB** (Blender, `npm run cuire:sprites`, `public/assets/sprites/`), composés en WebGL 2 (`src/render2d/`). `src/render3d/` est supprimé ; la décision du 14 septembre « conserver Three.js et WebGPU » ne vaut plus pour le jeu (`BRIEF.md`, « Sprites précalculés » ; `doc/18-rendu-sprites.md`). La production de modèles continue et garde ses règles de sources : **un GLB livré se recuit** (`npm run cuire:sprites -- --id <id>`). Les outils de fabrication (scripts de production, `scripts/production/lecture-glb.ts`, inspection de l'admin) gardent three classique. Ce qui suit est l'historique.

Rocher côtier intégré : 858 triangles, une primitive, 918 235 octets. Granit à treize pans chanfreinés, onze balanes implantées, douze volumes fermés ; pied au sol et quatre orientations mesurés sur le vrai lot instancié. Contrôle technique et typage réussis, aucun rendu. Trente-huit modèles livrés ; désert suivant après push. Archipel confirmé en ligne par SHA pour les quatre fichiers et inventaire. Repérage terrain futur uniquement dans tmp/production-sequentielle/raccordements-terrains.md, aucun TS partagé modifié.

Archipel poussé en `86af03e6`, raccordement runtime inclus ; publication distante à confirmer. Rocher côtier en cours, spécialiste `modele_34_rocher_cotier` : aucun upload le 20 septembre à 17:28 UTC, aucun actif, ancien candidat SHA `1f877d6b7ae0a9493c414a9c7605ed3bd3572c22ff9e5fb61adc0aedf6de8374`. Attendre FINAL gelé avant mesure instanciée, intégration et push. Root n’a plus de chantier TS partagé en attente ; seule suppression .vscode/settings.json reste étrangère et intouchable.

Rocher archipel intégré : 576 triangles, une primitive, 571 274 octets. Calcaire à cinq cavités et quatre coquillages, cinq volumes fermés. Contrôle du lot, chargement natif et typage réussis. Raccordement des rochers désormais inclus : sélection par biome, cache partagé, instanciation des sommets et grèves, pied GLB à Y=0 sans l’ancien enfouissement ; ressources rendues à la fermeture, y compris chargement annulé. Le relevé mesures-instanciation.json utilise le vrai GLB et quatre orientations sans moteur de rendu, source inchangée. Précharger tests/aides/webgpu-en-node.mjs aligne les imports Three comme dans Next, ne lance aucun test. Trente-sept modèles livrés ; rocher côtier suivant après push. Ville confirmée en ligne par SHA.

Ville poussée en `ca984d25`. Premier rocher en cours : `decor_rocher_archipel`, spécialiste `modele_33_rocher_archipel`, aucun upload vérifié le 20 septembre à 17:13 UTC, aucun actif ; candidat SHA `a8fbb7d8e0c7f190a6bbc1a9ae5b9f9a7382949f48ce8c49198eaa13f3ec60fa` (240 triangles). Attendre FINAL gelé, exécuter ensuite scripts/production/mesurer-rocher-runtime.ts sur son staging. Raccordement root prêt dans assets-environnement/index/decor/paysage/rochers-livres.ts, typage réussi avec la ville ; mesurer l’instance du vrai rocher puis inclure ces cinq fichiers et le script dans SON commit. Aucun rendu ni FPS.

Ville commune intégrée : 4 608 triangles, cinq primitives, 1 947 859 octets. Trois maisons, cour dégagée, auvent et râtelier ; 36 poses concordantes NumPy/GLTFLoader et 89 raccords ciblés, toits fermés et orientés. Contrôle de lot et typage réussis, aucun rendu. Trente-six modèles livrés : les 30 unités et les six bâtiments communs. Rocher archipel suivant après push, avec raccordement des rochers préparé séparément.

Usine poussée en `10a13883`, GLB et cinq PNG confirmés en ligne par SHA avec inventaire présent. Ville en cours : spécialiste `modele_32_ville`, aucun upload le 20 septembre à 17:00 UTC, ancien SHA `18daaaf44f866602c5a2f7eaed89b8789eeafa64dd1a987d0f30eb71442910e5`. Attendre FINAL gelé. Root a préparé le raccordement des futurs rochers dans assets-environnement/index/decor/paysage et rochers-livres.ts ; ces cinq fichiers et scripts/production/mesurer-rocher-runtime.ts restent HORS commit ville, jusqu’au contrôle du premier rocher. Sélection du biome, cache de GLB, fusion UV/normales puis instanciation ; baux et matériaux privés libérés, y compris chargement annulé. Pas encore mesuré avec un rocher livré.

Usine commune intégrée : 5 572 triangles, six primitives, 1 820 188 octets. Halle à sheds vitrés, palan sur chariot et cour de matériel ; 40 poses concordantes NumPy/GLTFLoader, 76 raccords ciblés, chaîne séparée et crochet fermé. Contrôle de lot et typage réussis, aucun rendu. Trente-cinq modèles livrés ; ville suivante après push. Le helper rochers-livres.ts reste séparé, non raccordé et non commité jusqu’au premier rocher.

Radar poussé en `108d12b7`, GLB et cinq PNG confirmés en ligne par SHA, inventaire présent. Usine en cours : `batiment_usine_base`, spécialiste `modele_31_usine`, absence d’upload vérifiée le 20 septembre à 16:45 UTC ; ancien SHA `e0ef8956fdd8eb1fb1d45c7dd2ba422ad793536e147ed66be0f030c192a7ee9c`. Attendre FINAL gelé. Root prépare séparément `src/render3d/rochers-livres.ts` (non raccordé, non commité) pour la future famille des rochers ; ne pas le mettre dans le commit usine/ville. Il fusionne les primitives statiques d’un matériau, conserve UV/normales et évite les tangentes non transformées des lots instanciés. Son raccordement et son contrôle restent à faire au premier rocher.
...
Cuirassé poussé en `1b2d23a`, puis bombardier (`39cc500`, 6 188 triangles, 1 552 728 octets) et brouilleur (`d33afbf`, 5 992 triangles, 1 592 511 octets). Ces lots sont vérifiés en ligne par leurs empreintes. Le contrôle de publication vérifie désormais aussi la présence dans `/api/modeles`. Les documents du staging sont préchargés avant toute mutation des alias : un README absent avait interrompu l'intégration locale du brouilleur ; l'ancien lot a été rétabli puis intégré complètement avant le push. Quinze modèles livrés à ce stade, drone d'observation suivant. Aucun test général ni contrôle visuel.

Drone d'observation poussé (`9efb0e8`, 3 188 triangles), puis intercepteur (`f9f9e19`, 3 380 triangles, 1 370 649 octets), tous deux vérifiés en ligne. Le helper `alleger` supprimait à tort les bornes des temps d'animation : seuls les GLB du brouilleur et de l'observateur en étaient affectés parmi les dix-sept livrés. Correctif du brouilleur poussé en `56adcc8` (+212 octets JSON ; BIN et six PNG identiques). Le contrôle binaire exige désormais FLOAT SCALAR et min/max cohérents ; le drone d'observation est corrigé en `34642e1` (+272 octets JSON, BIN et cinq PNG identiques). Les deux correctifs sont confirmés en ligne, puis la file reprend au drone marin. Rapports `correction-format.json` : différence JSON limitée aux bornes, aucune nouvelle géométrie. Utilitaire `retirer-kits-incompatibles.ts` disponible pour génie/méca/recon, inspection par défaut ; aucun kit retiré à ce stade. Voir aussi README production pour les raccordements de rendu encore nécessaires aux rochers et aux portraits.

Drone marin poussé en `d9e6ca7` : 3 488 triangles, six primitives, 1 374 735 octets. Ressort de l’antenne corrigé avant publication, spires séparées et jeu positif autour de l’âme mesurés dans le GLB. Quatre clips, 804 poses contrôlées et typage réussi ; aucune image ni validation artistique. GLB et cinq PNG confirmés en ligne par SHA, référence présente dans `/api/modeles`. Dix-huit modèles livrés ; ravitailleur suivant, un seul spécialiste actif. Le lecteur de clips conserve maintenant la pose finale de `hors_jeu` ; les autres clips ponctuels reviennent toujours au repos. Ce correctif accompagne le ravitailleur, sans suite de tests ni rendu.

Ravitailleur poussé en `0bdbba2` : 3 008 triangles, sept primitives, 1 428 392 octets. Deux os font tourner les rotors autour de leurs propres centres ; skin, chargement natif et clones mesurés sans rendu, 1 650 poses. Contrôle de lot et typage réussis ; correction partagée `hors_jeu` incluse. GLB et cinq PNG confirmés en ligne par SHA, présence dans l’inventaire du jeu vérifiée. Dix-neuf modèles livrés, chasseur furtif suivant. Aucun appareil réel détecté le 20 septembre via `xcrun devicectl list devices` et `adb devices` ; aucun relevé FPS téléphone effectué.

Furtif poussé en `2044132` : 5 088 triangles, cinq primitives, 1 479 343 octets. Voilure intégrée, cinq clips et 974 poses mesurées ; conduits arrière dégagés de la coque avant livraison, ressort sans traversées mesurées. Contrôle de lot et typage réussis, aucune image. GLB et cinq PNG confirmés en ligne par SHA, inventaire du jeu à jour. Vingt modèles livrés ; génie suivant, avec retrait à préparer des deux anciens kits actifs qui partagent sa géométrie précédente. Contrôle préalable du 20 septembre : aucun upload pour la base, `kit_fr_genie` ou `kit_lu_genie`, empreintes de géométrie/UV/nœuds/clips identiques à l’ancienne base ; aucun alias retiré avant que le nouveau lot soit prêt.

Les squelettes privés des figurines sont maintenant libérés au retrait en jeu, dans les duels et dans les aperçus du carnet/atelier. La vitrine libère aussi ses matériaux privés avant son moteur ; les géométries et textures partagées restent au cache. Correctif lié à l’arrivée des modèles articulés, couvert par le typage ciblé, sans suite de tests ni mesure de performances.
...
Lance-roquettes poussé en `c742d17`, GLB et cinq PNG confirmés en ligne par SHA, inventaire du jeu présent. Sous-marin suivant : `unite_sous_marin_base`, spécialiste `modele_26_sous_marin`, lecture distante sans upload le 20 septembre à 15:17 UTC, aucun actif ni kit. Ancien candidat SHA `1f0bd035995797145cd838eef9b20e8f64dbd5f5750a9c97eb137c11131afdde`, 596 triangles. Attendre FINAL gelé avant intégration ; trois gabarits a/b/c demandés.
...
Premier lot : anti-air source uploadée `bd7e4df5…`, 47 896 triangles, 11 841 950 octets, contrôle technique ok. Sources brute/préparée archivées immuables ; valeurs/cibles des cinq animations et masque conservés. Il remplace l'ancienne géométrie active de 2 704 triangles. Deuxième lot : artillerie source uploadée `38a5dbc6…`, 54 272 triangles, 9 352 100 octets, contrôle ok ; elle remplace l’ancienne géométrie active de 2 840 triangles. Les deux modèles sont livrés en commits individuels, chacun poussé avant le suivant. Inventaire distant finalement vérifié via la configuration du projet next-upload : 65 fiches consultées, cinq fiches avec uploads (révisions cohérentes), barge de source locale connue, aucun autre dépôt. Aucun jeton copié dans le suivi. Aucun test général, capture, contrôle visuel ni FPS téléphone. Intégrateur étendu à ces deux bases, avec garde de l'empreinte active avant préparation quand le maître HD n'a encore jamais été activé. Troisième spécialiste : automate méridien original paramétrique (Three.js/Python), 5 892 triangles, huit primitives, 1 635 455 octets, contrôle ok et activation directe ; sources reproductibles sous scripts/production/modeles/unite_meridien_automate_base. Il ne remplace aucun upload ni ancien lot. Quatrième spécialiste : transport original à 5 980 triangles, cinq primitives, 1 676 337 octets, contrôle ok ; ancien candidat de 844 triangles archivé, pas de source uploadée.
...
## Mobile et formations selon les PV — 14 septembre 2026

Après signalement de lenteurs dès Premier contact, le tactile reçoit un budget automatique : ratio 1, sans MSAA/ombres/post-traitement, boucle active plafonnée à 30 images/s, ambiance seule à 100 ms, météo limitée à 260 particules. Les textures des modèles passent en 1K avant envoi GPU quand ImageBitmap le permet, sans changer les fichiers, UV ou triangles LOD0 ; le téléchargement des sources reste inchangé. L'accueil tactile conserve le fond SVG sans monter sa partie 3D.

Le propriétaire demande seulement les deux textures des cases et une figurine par PV affiché. Le duel utilise désormais quatre images GPU temporaires : chaque modèle et chaque surface de case sont rendus une fois avec le renderer existant, puis deux formations instanciées affichent 0 à 10 exemplaires chacune. Les pertes retirent des figurines à l'impact ; cadence/riposte et règles du moteur inchangées. La carte n'est plus dessinée derrière. Il s'agit de figurines animées issues des GLB, pas d'articulations GLB calculées à chaque image. Ressources privées libérées en fermeture ; sources partagées préservées. Typage, build de production final et diff vérifiés ; aucun test, capture, contrôle visuel ou mesure FPS sur téléphone. Voir `doc/refonte/mobile-formations-combat.md`.
