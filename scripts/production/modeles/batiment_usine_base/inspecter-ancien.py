...
assert r['sha256Glb']=='e0ef8956fdd8eb1fb1d45c7dd2ba422ad793536e147ed66be0f030c192a7ee9c' and r['triangles']==1164 and r['octetsGlb']==137868
...
provenance={'id':ID,'verificationLocale':[{'chemin':str(p),'existe':p.exists(),'fichiers':[str(f) for f in p.rglob('*') if f.is_file()] if p.is_dir() else []} for p in paths],'maitreLocalIdentifie':any(p.exists() for p in paths),'lectureDistanteCommuniquee':{'date':'2026-09-20T16:45:12.134Z','nombreDepots':0,'origine':'Coordinateur, lecture authentifiée réussie ; aucun secret copié.'},'ancienCandidat':'inspection-ancien-candidat.json','candidatPreserve':True}
