...
PALETTE = [
    ...
    (62,69,72),    # peinture technique graphite des équipements navals
    (36,39,40),    # caoutchouc
    (149,156,160), # aluminium/inox brossé
    (23,53,65),    # lentille teintée sans reflet peint
    (127,136,141), # panneaux techniques de coque et passerelle
...
def fabriquer(sortie: Path):
    ...
    for role in range(16):
        rng = np.random.default_rng(920473 + role)
        ...
        if genre == 8:
            # Nervures pressées du pont : relief de matière, jamais une ombre.
