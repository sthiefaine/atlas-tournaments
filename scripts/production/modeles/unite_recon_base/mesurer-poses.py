...
for a in doc['animations']:
    ...
    mesures={g:{'min':np.full(3,np.inf),'max':np.full(3,-np.inf),'rayon':0} for g in gabarits}
    for t in instants:
        pose={}
        ...
        for g,echelle in gabarits.items():
            ...
            m=mesures[g];m['min']=np.minimum(m['min'],points_runtime.min(axis=0));m['max']=np.maximum(m['max'],points_runtime.max(axis=0));m['rayon']=max(m['rayon'],float(np.linalg.norm(points_runtime[:,[0,2]],axis=1).max()))
    ...
    for g,echelle in gabarits.items():
        m=mesures[g];borne_g=max(borne_temoin*max(echelle),m['rayon']+max(np.diff(instants))/2*vitesse*max(echelle))
...
assert np.all(abs(dimensions-np.array([.5,.4,.7]))<np.array([.06,.06,.06]))
...
rapport_g={'id':identifiant,'ordreTransformations':'RY(pi/2) puis S ; Xruntime=Zfichier*sx, Yruntime=Yfichier*sy, Zruntime=-Xfichier*sz','echelles':gabarits,'clipsParGabarit':mesures_gabarits,'methode':'193 instants plus toutes clés ; majoration continue par vitesse LINEAR/slerp et demi-pas multipliée par la norme opérateur max(S). Témoin borné séparément par sphère de chaîne.','limites':rapport['limites'],'approbationArtistique':False}
