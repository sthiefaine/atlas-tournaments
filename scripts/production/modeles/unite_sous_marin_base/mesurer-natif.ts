...
async function mesurer(){
 ...
 const moduleOriginal=original.getObjectByName('module_antenne')!,moduleCopie=exemplaire.getObjectByName('module_antenne')!;
 ...
 if(moduleCopie.getWorldPosition(new THREE.Vector3()).distanceTo(centre)>1e-8)throw new Error('Pivot antenne mobile');
 ...
 const rapport={id,methode:'GLTFLoader natif, seules références de cartes retirées en mémoire ; SkeletonUtils.clone et AnimationMixer sans rendu. Géométrie et clips inchangés.',clonesNoeudsIndependants:true,pivotAntenneFixeDansCorps:true,pariteNumpyDeuxMicrometres:true,clips,limites:['Textures non chargées dans ce contrôle natif ; références contrôlées séparément.','Chaque clip isolé, transitions exclues.','Aucun rendu ni appréciation artistique.'],approbationArtistique:false};
