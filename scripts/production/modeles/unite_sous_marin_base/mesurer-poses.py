...
identifiant='unite_sous_marin_base'
...
gabarits={'a':[.9,.98,1.06],'b':[1,1,1],'c':[1.14,1.06,.95]}
mesures_gabarits={g:[] for g in gabarits}
clips=[]
for a in doc['animations']:
    ...
    mesures={g:{'min':np.full(3,np.inf),'max':np.full(3,-np.inf),'rayon':0,'rayonsNoeuds':{i:0. for i in points}} for g in gabarits}
    for t in instants:
        pose={}
        ...
        for g,echelle in gabarits.items():
            points_runtime=sommets[:,[2,1,0]]*np.array([1,1,-1])*np.array(echelle)
            m=mesures[g]
            for i,pp in points.items():
                pr=(pp@matrices[i][:3,:3].T+matrices[i][:3,3])[:,[2,1,0]]*np.array([1,1,-1])*np.array(echelle)
                m['rayonsNoeuds'][i]=max(m['rayonsNoeuds'][i],float(np.linalg.norm(pr[:,[0,2]],axis=1).max()))
            m['min']=np.minimum(m['min'],points_runtime.min(axis=0));m['max']=np.maximum(m['max'],points_runtime.max(axis=0));m['rayon']=max(m['rayon'],float(np.linalg.norm(points_runtime[:,[0,2]],axis=1).max()))
    vitesses=vitesse_limite(canaux);vitesse=max(vitesses.values());borne=max(borne_temoin,max(mesures['b']['rayonsNoeuds'][i]+max(np.diff(instants))/2*v for i,v in vitesses.items()))
    assert borne<.5 and minimum[1]>-1e-6,(a['name'],borne,minimum)
    for g,echelle in gabarits.items():
        m=mesures[g];borne_g=max(borne_temoin*max(echelle),max(m['rayonsNoeuds'][i]+max(np.diff(instants))/2*v*max(echelle) for i,v in vitesses.items()))
        assert borne_g<.5 and m['min'][1]>-1e-6,(g,a['name'],borne_g,m)
        mesures_gabarits[g].append({'nom':a['name'],'poses':len(instants),'min':m['min'].tolist(),'max':m['max'].tolist(),'rayonEchantillonne':m['rayon'],'rayonMajoreContinu':borne_g,'jeuVoisinsTousCaps':1-2*borne_g})
...
assert np.all(abs(dimensions-np.array([.62,.50,.85]))<=np.array([.07,.07,.07]))
rapport={'id':identifiant,'methode':'Relecture GLB NumPy : sommets, translations LINEAR et rotations slerp ; 193 instants et toutes clés. Comparaison aux mesures Three.js du GLB exporté à 2 µm près. Majoration continue de toute géométrie sauf témoin par nœud et sa vitesse maximale de translation/slerp et demi-pas. Le témoin rétractable est majoré par sa sphère locale autour de la chaîne corps sans translation horizontale animée.','clips':clips,'borneAnalytiqueTemoin':borne_temoin,'bornesStatiques':{'min':mini.tolist(),'max':maxi.tolist(),'dimensions':dimensions.tolist()},'portee':'GLB neutre, gabarits runtime a/b/c après RY(pi/2), tous caps grâce à la borne radiale.','limites':['Contrôle numérique sans rendu ni approbation artistique.','Sol mesuré aux échantillons ; rayon majoré continûment.','Chaque clip lu séparément ; mélanges entre clips non mesurés.','Les collisions internes ne sont pas certifiées par cette mesure d’enveloppe.','Les trois gabarits runtime sont contrôlés ; aucune affirmation FPS.']}
...
rapport_g={'id':identifiant,'ordreTransformations':'RY(pi/2) puis S ; Xruntime=Zfichier*sx, Yruntime=Yfichier*sy, Zruntime=-Xfichier*sz','echelles':gabarits,'clipsParGabarit':mesures_gabarits,'methode':'193 instants plus toutes clés ; majoration continue par nœud, vitesse LINEAR/slerp et demi-pas multipliée par la norme opérateur max(S). Témoin borné séparément par sphère de chaîne.','limites':rapport['limites'],'approbationArtistique':False}
(sortie/'mesures-gabarits.json').write_text(json.dumps(rapport_g,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'rayonsMajoresParGabarit':{g:max(c['rayonMajoreContinu'] for c in cs) for g,cs in mesures_gabarits.items()}}))
