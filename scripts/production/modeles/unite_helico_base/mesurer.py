...
assert rapport['triangles']<=6000 and rapport['trianglesDegeneres']==0 and rapport['uvTrianglesDegeneres']==0 and rapport['normalesInversees']==0,rapport
...
# Vérifie le binaire des pistes et non seulement les valeurs de génération.
...
rapport['rotor']={}
for animation in doc['animations']:
    canaux=[c for c in animation['channels'] if doc['nodes'][c['target']['node']]['name']=='base']
    assert len(canaux)==1 and canaux[0]['target']['path']=='rotation'
    s=animation['samplers'][canaux[0]['sampler']]
    q=acc(s['output']); t=acc(s['input'])[:,0]
    assert np.all(np.diff(t)>0) and np.max(abs(np.linalg.norm(q,axis=1)-1))<1e-6
    # q et -q sont la même orientation ; les deltas de phase prennent le chemin local.
    a=np.unwrap(2*np.arctan2(q[:,1],q[:,3])); delta=np.diff(a)
    rapport['rotor'][animation['name']]={'nombreCles':len(t),'tours':round(float((a[-1]-a[0])/(2*np.pi)),6),'angleMaxEntreClesDegres':float(np.max(abs(delta))*180/np.pi),'axeYSeulement':bool(np.all(q[:,[0,2]]==0)),'aucunRetourInverse':bool(np.all(delta>-1e-6)),'arretFinal':bool(np.array_equal(q[-2],q[-1]))}
    assert np.max(abs(delta))<np.pi and np.all(delta>-1e-6) and np.all(q[:,[0,2]]==0)
assert rapport['noeuds']==['base','corps','module_nacelle','racine','socle']
assert rapport['materiaux']==['mat_corps','mat_details'] and rapport['interpolationClips']==['LINEAR']
assert rapport['rotor']['hors_jeu']['arretFinal']
rapport['octetsGlbEtPng']=sum(p.stat().st_size for p in sortie.iterdir() if p.suffix in ['.glb','.png'])
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'rotor':rapport['rotor'],'octetsGlbEtPng':rapport['octetsGlbEtPng']},ensure_ascii=False,indent=2))
