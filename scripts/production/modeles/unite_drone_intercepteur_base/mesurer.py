...
rapport['reculLanceursDansTir']=any(doc['nodes'][c['target']['node']]['name']=='base' and c['target']['path']=='translation' and np.ptp(acc(a['samplers'][c['sampler']]['output'])[:,1])>.021 for a in doc['animations'] if a['name']=='tir' for c in a['channels'])
...
assert rapport['noeuds']==['base','corps','module_radar','racine','socle'] and len(doc['materials'])==2
...
assert rapport['indicateurCacheFinHorsJeu'] and rapport['radarAnimeDansTousClips'] and rapport['reculLanceursDansTir']
