...
writeFileSync(path.join(sortie, 'mesures-poses-three-glb.json'), JSON.stringify({ id, methode: 'Relecture des positions et pistes depuis le GLB exporté ; Three.js sans texture ni rendu, 193 instants plus clés exactes.', clips, approbationArtistique: false }, null, 2) + '\n');
console.log(JSON.stringify({ clips: clips.length, poses: clips.reduce((n, c) => n + c.nombreEchantillons, 0), relectureGLB: true }));
