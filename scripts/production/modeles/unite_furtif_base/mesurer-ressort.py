...
segments=np.rint((uv[:,0]*4-3-.065)/.87*144).astype(int)
segments=segments[indices].min(axis=1)
triangles=p[indices]
# Le centre de l'âme est le pivot local, rayon 10 mm. Le minimum radial
...
radial[interieur]=0
jeu_ame=float(radial.min()-.010)
assert jeu_ame>0,jeu_ame
# Faces de portions distinctes : au moins 32 des 144 segments (2/3 de tour).
...
for i in range(len(indices)):
 j=np.arange(i+1,len(indices));j=j[np.abs(segments[j]-segments[i])>=32]
...
rapport={'id':identifiant,'approbationArtistique':False,'parametresMetres':{'rayonCentral':.035,'diametreFil':.021,'nombreTours':3,'hauteur':.090,'pas':.030,'diametreAme':.020},'glbRelu':True,'trianglesRessort':len(indices),'pairesFacesMesurees':len(ia),'seuilEliminationParAabbMetres':.020,'jeuAmeBorneInferieureMetres':jeu_ame,'jeuEntreSpiresMetres':jeu_spires,'intersectionsEntreSpires':intersections,'methode':'GLB exporté : fil identifié par sa cellule UV. Minimum radial continu sur les triangles projetés, moins le rayon de l’âme cylindrique pleine de 10 mm. Distance triangle-triangle (sommets/faces et arêtes/arêtes, avec détection de traversées) entre portions de fil éloignées d’au moins 32 des 144 segments de l’hélice.','limites':['Contrôle local du ressort, pas de certification complète des collisions internes.','Les portions contiguës du même fil sont raccordées intentionnellement.','Le module antenne est une articulation rigide ; ces jeux internes sont invariants dans ses cinq clips.']}
