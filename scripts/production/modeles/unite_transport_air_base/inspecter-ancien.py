...
assert r['sha256Glb']=='379ea6aefb4b100e339860d638018e4494d140bcee285027d3aef77fd4191659' and r['triangles']==576 and r['octetsGlb']==70316
...
provenance={'id':ID,'verificationLocale':[{'chemin':str(p),'existe':p.exists(),'fichiers':[str(f) for f in p.rglob('*') if f.is_file()] if p.is_dir() else []} for p in paths],'maitreLocalIdentifie':any(p.exists() for p in paths),'lectureDistanteCommuniquee':{'date':'2026-09-20T15:32:10.512Z','nombreDepots':0,'origine':'Coordinateur, lecture authentifiée réussie ; aucun secret copié.'},'ancienCandidat':'inspection-ancien-candidat.json','candidatPreserve':True}
