#!/usr/bin/env python3
"""Revue compacte ; recoupe SHA du lot, du typage et du candidat préservé."""
from pathlib import Path
from datetime import datetime,timezone
import json,hashlib,sys
ID='unite_transport_air_base';source=Path(__file__).resolve().parent;racine=source.parents[3]
...
r={'id':ID,'dateRevueUtc':datetime.now(timezone.utc).isoformat(),'statut':'pret_a_integrer_par_coordinateur','approbationArtistique':False,
 ...
 'mesuresResume':{'posesParGabarit':sum(c['nombreEchantillons'] for c in poses['clips']),'pariteNumpyThreeNatifToleranceMetres':.000002,'rayonsMajoresABC':rayons,'espaceDeuxVoisinsABC':{g:1-2*r for g,r in rayons.items()},'raccordsIntentionnels':len(jeux['raccords']),'rotor':jeux['rotor'],'jeuMinimumCrochetBrasAuxPoses':min(c['crochetBrasDistanceTrianglesMin'] for c in jeux['posesModules']),'palesGrueMinimumSeparationYAuxPoses':min(c['palesGrueSeparationY'] for c in jeux['posesModules']),'grueCabineMinimumSeparationZAuxPoses':min(c['grueCabineSeparationZ'] for c in jeux['posesModules']),'sondesPlancherOuvert':len(jeux['sondesPlancherOuvert']),'racineNonAnimee':not geo['racineAnimee'],'tempsAnimationsMinMaxConserves':geo['bornesTempsAnimationConservees'],'bouclesPoseEtVitesse':True,'horsJeuTientPoseEtMasqueTemoin':geo['horsJeuTientPoseFinale'] and geo['indicateurCacheFinHorsJeu']},
 'correctionsAvantGel':['Patins cintrés remontés du dépassement réel mesuré de leur courbe, pour un minimum Y exactement nul.','Coude de grue replié dans le sens qui conserve le crochet au-dessus du bras ; seuil de garde choisi de 3 mm, pas une exigence chiffrée de la spec.'],
 'limites':['Aucun rendu, capture, contrôle visuel ni approbation artistique.','Aucun test général, build, téléphone ou FPS.','Clips isolés ; transitions et mélanges exclus.','Sol et jeux aux poses, rayon majoré continûment pour a/b/c.','Distances locales et sondes ponctuelles ; aucune certification globale des collisions.','Capteurs opaques, quatre pales rigides et atlas répété.','Aucun HD ni bake ; lisibilité à apprécier humainement.'],
