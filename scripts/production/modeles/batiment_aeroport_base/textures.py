...
ID='batiment_aeroport_base'
out=Path(sys.argv[1]) if len(sys.argv)>1 else Path('tmp/production-sequentielle')/ID
# 0 béton dalle ; 1 apron anneau ; 2 peinture équipe ; 3 métal ; 4 vitrage cabine ;
# 5 tôle claire ; 6 rail graphite ; 7 caoutchouc ; 8 porte ; 9 tissu ; 10 feux ; 11 vitre camion.
palette=[(185,182,174),(195,193,184),(170,170,170),(145,152,153),(33,67,76),(198,198,194),(62,67,68),(34,36,36),(129,139,140),(220,116,44),(231,187,112),(26,48,55),(185,182,174),(185,182,174),(185,182,174),(185,182,174)]
rough=[224,224,166,113,47,182,193,228,181,224,57,51,224,224,224,224]
...
for role in range(16):
 rng=np.random.default_rng(289133+role);v,u=np.mgrid[0:S,0:S]/S
 ...
 if role==1:
  r=np.hypot(u-.5,v-.5);ring=(r>.333)&(r<.358)
  pigment[ring]=np.array([225,222,207])+grain[ring,None]*1.4
  height[ring]+=.000045
 if role in [2,5,8]:
 ...
 if role==9:
  # Bandes de toile fonctionnelles, sans symbole ; elles suivent la longueur réelle.
  band=np.floor((v-.065)/.87*5).astype(int)%2==0
  pigment[band]=np.array([224,218,197])+grain[band,None]*1.8
  height+=np.sin(u*2*np.pi*100)*.00004+np.sin(v*2*np.pi*98)*.00004
