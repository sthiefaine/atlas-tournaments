...
rapport={'triangles':0,'primitives':0,'trianglesDegeneres':0,'uvTrianglesDegeneres':0,'normalesInversees':0,'normalesErreurMax':0.,'tangentesErreurMax':0.,'orthogonaliteErreurMax':0.,'valeursFinies':True,'uvDansAtlas':True}
...
assert rapport['triangles']<=9000 and rapport['trianglesDegeneres']==0 and rapport['uvTrianglesDegeneres']==0 and rapport['normalesInversees']==0,rapport
...
rapport['reculMarqueursZ']=[]
for a in doc['animations']:
    if a['name']!='tir':continue
    for c in a['channels']:
        if doc['nodes'][c['target']['node']]['name']=='module_canon_long' and c['target']['path']=='translation':
            v=acc(a['samplers'][c['sampler']]['output']); delta=v-v[0]
            rapport['reculMarqueursZ']=delta[:,2].tolist()
            assert np.all(delta[:,:2]==0) and np.min(delta[:,2])<-.02 and np.max(delta[:,2])==0 and np.array_equal(v[0],v[-1])
assert rapport['noeuds']==['base','corps','module_canon_long','module_tourelle','racine','socle'] and len(doc['materials'])==2
assert rapport['materiaux']==['mat_corps','mat_details'] and rapport['interpolationClips']==['LINEAR']
assert rapport['quaternionsUnitaires'] and rapport['tempsStrictementCroissants'] and rapport['indicateurCacheFinHorsJeu'] and rapport['reculMarqueursZ']
rapport['octetsGlbEtPng']=sum(p.stat().st_size for p in sortie.iterdir() if p.suffix in ['.glb','.png'])
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'reculMarqueursZ':rapport['reculMarqueursZ'],'octetsGlbEtPng':rapport['octetsGlbEtPng']},ensure_ascii=False,indent=2))
# Contrôle des données PBR décodées et de leur raccord aux deux matériaux glTF.
normal_png=np.array(Image.open(sortie/f'{identifiant}_normale.png')).astype(float)/255*2-1
normes_png=np.linalg.norm(normal_png,axis=2)
rapport['normalePngErreurNormeMax']=float(np.max(abs(normes_png-1)))
rapport['normalePngZMin']=float(normal_png[:,:,2].min())
...
rapport['masqueFractionBlanche']=float(np.mean(masque==255))
rapport['pbrCartesRaccordees']=all(m['pbrMetallicRoughness']['baseColorTexture']['index']==0 and m['pbrMetallicRoughness']['metallicRoughnessTexture']['index']==2 and m['normalTexture']['index']==1 for m in doc['materials'])
rapport['texturesCanaux']={i['name']:i['uri'] for i in doc['images']}
assert rapport['normalePngErreurNormeMax']<.007 and rapport['normalePngZMin']>0 and rapport['pbrCartesRaccordees']
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
