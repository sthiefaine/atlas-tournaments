...
rapport['bornesTempsAnimationConservees']=all(all(k in doc['accessors'][s['input']] for k in ['min','max']) and np.allclose(doc['accessors'][s['input']].get('min',[]),[float(acc(s['input'])[0,0])]) and np.allclose(doc['accessors'][s['input']].get('max',[]),[float(acc(s['input'])[-1,0])]) for a in doc['animations'] for s in a['samplers'])
...
assert all(n in rapport['noeuds'] for n in ['racine','corps','base','socle','module_radar']) and len(doc['materials'])==2
...
assert rapport['osHumainsDansPeau']=={'peaux':1,'os':28}
rapport['masqueRolesNeutres']={}
for role,nom in [(2,'caoutchouc'),(3,'metal_nu'),(4,'verre'),(8,'peau_technicien'),(9,'peau_technicienne'),(10,'cheveux_bruns'),(11,'cheveux_sombres')]:
