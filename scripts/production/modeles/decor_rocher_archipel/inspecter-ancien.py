...
r={'id':ID,'sha256Glb':hashlib.sha256(b).hexdigest(),'octetsGlb':len(b),'triangles':sum(len(acc(p['indices']))//3 for m in d['meshes'] for p in m['primitives']),'noeuds':[n['name'] for n in d['nodes']],'animations':[a['name'] for a in d.get('animations',[])],'textures':[],'controleVisuel':False,'importeDansCreation':False}
...
assert r['sha256Glb']=='a8fbb7d8e0c7f190a6bbc1a9ae5b9f9a7382949f48ce8c49198eaa13f3ec60fa' and r['triangles']==240 and r['octetsGlb']==26868
assert not (Path('public/assets/modeles')/f'{ID}_lod0.glb').exists(); r['actifAbsent']=True
...
paths=[Path('assets/sources')/ID,Path('assets/livraisons')/ID/'source.json',Path('assets/livraisons')/ID/'maitre.json',Path('tmp/optimisation-lod0')/ID/'maitre.json',sortie/'maitre.json']
provenance={'id':ID,'verificationLocale':[{'chemin':str(p),'existe':p.exists(),'fichiers':[str(f) for f in p.rglob('*') if f.is_file()] if p.is_dir() else []} for p in paths],'maitreLocalIdentifie':any(p.exists() for p in paths), 'rechercheDownloads': {'fichiersRochers':['plaine_field_boulder.glb','plaine_field_boulder_detailed.glb','decor_rocher_plaine_lod0_fixed.glb','decor_rocher_plaine_complete','decor_rocher_plaine_fixed_complete'], 'conclusion':'Uniquement plaine, exclue de cette commande ; aucun fichier archipel.', 'portee':'Recherche des noms de fichiers 3D/archives dans Downloads et inventaire assets/sources, sans présumer le contenu des fichiers non identifiés.'},'lectureDistanteCommuniquee':{'date':'2026-09-20T17:13:51.127Z','nombreDepots':0,'origine':'Coordinateur, lecture authentifiée réussie ; aucun secret copié.'},'ancienCandidat':'inspection-ancien-candidat.json','candidatPreserve':True}
