...
async function main() {
  const id = 'decor_rocher_archipel', out = `tmp/production-sequentielle/${id}`;
  ...
  if (meshes !== 1 || triangles !== 576 || error > .000002 || loaded.animations.length) throw new Error('Lecture native divergente');
