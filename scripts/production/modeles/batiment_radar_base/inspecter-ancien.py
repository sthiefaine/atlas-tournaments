...
assert r['sha256Glb']=='cf3e5f30e142acbdc4019c1cc30d1317dff872ef25919e7ac571ed34607cf8c5' and r['triangles']==684 and r['octetsGlb']==81260
...
provenance={'id':ID,'verificationLocale':[{'chemin':str(p),'existe':p.exists(),'fichiers':[str(f) for f in p.rglob('*') if f.is_file()] if p.is_dir() else []} for p in paths],'maitreLocalIdentifie':any(p.exists() for p in paths),'lectureDistanteCommuniquee':{'date':'2026-09-20T16:22:21.046Z','nombreDepots':0,'origine':'Coordinateur, lecture authentifiée réussie ; aucun secret copié.'},'ancienCandidat':'inspection-ancien-candidat.json','candidatPreserve':True}
