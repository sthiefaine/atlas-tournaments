...
PALETTE = [
    (167,167,167), # cabine, capots et coffrets couleur d'équipe, gris strict
    (57,64,67),    # sous-châssis graphite peint
    (36,39,40),    # caoutchouc
    (141,148,153), # aluminium/titane brossé
    (23,53,65),    # lentille teintée sans reflet peint
    (117,125,128), # panneaux gris techniques
    (49,56,59),    # grille de refroidissement
    (216,155,71),  # verre ambré du témoin (pas une couleur de camp)
    (79,84,85), (59,65,69), (36,39,40), (141,148,153),
...
def fabriquer(sortie: Path):
    ...
    emission = np.zeros((taille,taille,3),dtype=np.uint8)
    for role in range(16):
        rng = np.random.default_rng(94281 + role)
        ...
        if role == 15: emission[s] = [255,164,62]
    cartes = {
        ...
        'emission': Image.fromarray(emission).resize((512,512),Image.Resampling.NEAREST),
    }
