...
# Balayage autonome du module radar pour tout angle de cap.
minimum=np.full(3,np.inf);maximum=np.full(3,-np.inf);rayon=0
for deg in range(361):
    a=np.deg2rad(deg)/2;sommets,_=calculer({noeuds['module_radar']:{'rotation':[0,np.sin(a),0,np.cos(a)]}})
    minimum=np.minimum(minimum,sommets.min(axis=0));maximum=np.maximum(maximum,sommets.max(axis=0));rayon=max(rayon,float(np.linalg.norm(sommets[:,[0,2]],axis=1).max()))
# Borne analytique exacte sur la distance horizontale du pivot plus le rayon local.
_,matrices=calculer({});radar=noeuds['module_radar'];pivot=matrices[radar][:3,3]
rayon_module=float(np.linalg.norm(pivot[[0,2]])+np.linalg.norm(points[radar][:,[0,2]],axis=1).max())
borne_balayage=max(rayon,rayon_module)
assert borne_balayage<.5
rapport={'id':identifiant,'methode':'Relecture GLB NumPy : sommets, translations LINEAR et rotations slerp ; 193 instants et toutes clés. Comparaison aux mesures Three.js avant export à 2 µm près. Majoration continue par vitesse maximale des articulations et demi-pas entre échantillons.','clips':clips,'borneAnalytiqueTemoin':borne_temoin,'balayageRadar':{'echantillons':361,'min':minimum.tolist(),'max':maximum.tolist(),'rayonHorizontalMax':rayon,'rayonAnalytiqueModule':rayon_module,'rayonHorizontalBorneContinue':borne_balayage},'limites':['Contrôle numérique sans rendu ni approbation artistique.','Sol mesuré aux échantillons ; rayon majoré continûment.','Chaque clip lu séparément ; mélanges entre clips non mesurés.','Les collisions internes ne sont pas certifiées par cette mesure d’enveloppe.']}
(sortie/'mesures-poses-glb.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'clips':len(clips),'poses':sum(c['nombreEchantillons'] for c in clips),'rayonHorizontalMax':max(c['rayonHorizontalMax'] for c in clips),'rayonHorizontalMajorationContinue':max(c['rayonHorizontalMajorationContinue'] for c in clips),'balayageRadar':borne_balayage,'pariteExport':True}))
