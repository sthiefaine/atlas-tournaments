...
identifiant='unite_brouilleur_base'
...
rapport['radarAnimeDansTousClips']=all(any(doc['nodes'][c['target']['node']]['name']=='module_radar' and c['target']['path']=='rotation' and np.ptp(acc(a['samplers'][c['sampler']]['output']),axis=0).max()>.01 for c in a['channels']) for a in doc['animations'])
rapport['antenneAnimeeDansTousClips']=all(any(doc['nodes'][c['target']['node']]['name']=='module_antenne' and c['target']['path']=='rotation' for c in a['channels']) for a in doc['animations'])
rapport['emissionLimiteeAuSocle']=True
emission=np.array(Image.open(sortie/f'{identifiant}_emission.png'))
for n in doc['nodes']:
    if 'mesh' not in n:continue
    for prim in doc['meshes'][n['mesh']]['primitives']:
        uv=acc(prim['attributes']['TEXCOORD_0']);indices=acc(prim['indices']).astype(int).reshape(-1,3)
        centres=np.mean(uv[indices],axis=1)
        # Sommets + centroïdes : chaque primitive reste dans sa tuile d’atlas.
        tous=np.concatenate([uv,centres]);pixels=np.minimum(np.floor(tous*512).astype(int),511)
        e=emission[pixels[:,1],pixels[:,0]]
        if n['name']=='socle':rapport['emissionLimiteeAuSocle'] &= bool(np.all(e.max(axis=1)>0))
        else:rapport['emissionLimiteeAuSocle'] &= bool(np.all(e==0))
...
rapport['paraboleConcaviteMetres']=.05
rapport['paraboleMesure']='Surface analytique z=-0,026+0,05*(r/0,145)^2, doublée à 12 mm ; profondeur dans fabrication.json.'
rapport['ressortTours']=3
rapport['ressortPasMetres']=.024
rapport['ressortDiametreFilMetres']=.0222
assert rapport['noeuds']==['base','corps','module_antenne','module_radar','racine','socle'] and len(doc['materials'])==2
assert rapport['materiaux']==['mat_corps','mat_details'] and rapport['quaternionsUnitaires'] and rapport['tempsStrictementCroissants']
assert rapport['indicateurCacheFinHorsJeu'] and rapport['radarAnimeDansTousClips'] and rapport['antenneAnimeeDansTousClips'] and rapport['emissionLimiteeAuSocle']
assert rapport['normalePngLongueurErreurMax']<.008 and rapport['normalePngZMinimum']>0
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'canauxVerifies':True,'emissionLimiteeAuSocle':rapport['emissionLimiteeAuSocle'],'radarAnimeDansTousClips':rapport['radarAnimeDansTousClips']}))

# Profondeur et peau arrière mesurées après annulation de l’inclinaison de repos.
radar=next(n for n in doc['nodes'] if n['name']=='module_radar')
p=np.concatenate([acc(pr['attributes']['POSITION']) for pr in doc['meshes'][radar['mesh']]['primitives']])
a=.55;rotation_inverse=np.array([[1,0,0],[0,np.cos(a),-np.sin(a)],[0,np.sin(a),np.cos(a)]])
p=p@rotation_inverse.T;r=np.linalg.norm(p[:,:2],axis=1)
centre=p[(r<1e-6)&(abs(p[:,2]+.026)<1e-6),2];bord=p[(abs(r-.145)<1e-6)&(abs(p[:,2]-.024)<1e-6),2]
arriere=p[(r<1e-6)&(abs(p[:,2]+.038)<1e-6),2]
assert len(centre)>0 and len(bord)>0 and len(arriere)>0
rapport['paraboleConcaviteMesureeMetres']=float(bord.mean()-centre.mean())
rapport['paraboleEpaisseurMesureeMetres']=float(centre.mean()-arriere.mean())
rapport['paraboleMesure']='Sommets centre/bord/centre arrière du GLB, rotation de repos annulée.'
assert abs(rapport['paraboleConcaviteMesureeMetres']-.05)<1e-6 and abs(rapport['paraboleEpaisseurMesureeMetres']-.012)<1e-6
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
