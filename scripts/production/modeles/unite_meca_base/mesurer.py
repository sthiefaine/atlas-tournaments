...
rapport={'triangles':0,'primitives':0,'trianglesDegeneres':0,'trianglesSuperposes':0,'uvTrianglesDegeneres':0,'normalesInversees':0,'normalesErreurMax':0.,'tangentesErreurMax':0.,'orthogonaliteErreurMax':0.,'valeursFinies':True,'uvDansAtlas':True,'uvTrianglesDansUneCellule':True,'margeAtlasPixelsMinimum':1024.}
for mesh in doc['meshes']:
    for prim in mesh['primitives']:
        ...
        cellules=np.floor(uv[idx]*4).astype(int)
        rapport['uvTrianglesDansUneCellule'] &= bool(np.all(cellules==cellules[:,:1,:]))
        local=uv*4-np.floor(uv*4)
        rapport['margeAtlasPixelsMinimum']=min(rapport['margeAtlasPixelsMinimum'],float(np.min(np.minimum(local,1-local))*256))

rapport['imagesExternes']=all(i.get('uri')==f'{identifiant}_{i["name"]}.png' and 'bufferView' not in i for i in doc['images'])
...
assert rapport['triangles']<=4000 and rapport['trianglesDegeneres']==0 and rapport['trianglesSuperposes']==0 and rapport['uvTrianglesDegeneres']==0 and rapport['normalesInversees']==0,rapport
assert rapport['uvTrianglesDansUneCellule'] and rapport['margeAtlasPixelsMinimum']>=16
...
assert rapport['dureesClipsMs']=={'repos':2400,'deplacement':1000,'tir':700,'touche':500,'hors_jeu':900,'capture':1300} and rapport['horsJeuTientPoseFinale'],rapport
...
rapport['poidsRigidesNormalises']=all(np.all(acc(p['attributes']['WEIGHTS_0'])==np.array([1,0,0,0])) for n in doc['nodes'] if n.get('name')=='corps' for p in doc['meshes'][n['mesh']]['primitives'])
rapport['racineIdentite']=all(np.allclose(n.get(k,v),v) for n in doc['nodes'] if n['name']=='racine' for k,v in [('translation',[0,0,0]),('rotation',[0,0,0,1]),('scale',[1,1,1])])
...
assert all(n in rapport['noeuds'] for n in ['racine','corps','base','socle','module_lance_roquettes']) and len(doc['materials'])==2
...
assert rapport['indicateurCacheFinHorsJeu'] and rapport['poidsRigidesNormalises'] and rapport['racineIdentite']
assert rapport['normalePngLongueurErreurMax']<.008 and rapport['normalePngZMinimum']>0
rapport['osHumainsDansPeau']={'peaux':len(doc['skins']),'os':len(doc['skins'][0]['joints'])}
assert rapport['osHumainsDansPeau']=={'peaux':1,'os':27}
rapport['masqueRolesNeutres']={}
for role,nom in [(2,'caoutchouc'),(3,'metal_nu'),(4,'verre'),(8,'peau_porteuse'),(9,'peau_equipier'),(10,'cheveux_porteuse'),(11,'cheveux_equipier')]:
    y,x=(role//4)*128,(role%4)*128
    valeurs=[int(v) for v in np.unique(masque[y:y+128,x:x+128])]
    assert valeurs==[0],(nom,valeurs)
    rapport['masqueRolesNeutres'][nom]=valeurs
rapport['semellesPoseNeutreCibleMetres']=.018
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'canauxVerifies':True,'osHumainsDansPeau':rapport['osHumainsDansPeau']}))
