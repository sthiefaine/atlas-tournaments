...
assert r['sha256Glb']=='18daaaf44f866602c5a2f7eaed89b8789eeafa64dd1a987d0f30eb71442910e5' and r['triangles']==1036 and r['octetsGlb']==129500
...
provenance={'id':ID,'verificationLocale':[{'chemin':str(p),'existe':p.exists(),'fichiers':[str(f) for f in p.rglob('*') if f.is_file()] if p.is_dir() else []} for p in paths],'maitreLocalIdentifie':any(p.exists() for p in paths),'lectureDistanteCommuniquee':{'date':'2026-09-20T17:00:55.074Z','nombreDepots':0,'origine':'Coordinateur, lecture authentifiée réussie ; aucun secret copié.'},'ancienCandidat':'inspection-ancien-candidat.json','candidatPreserve':True}
