...
async function mesurer(){
 ...
 const original=gltf.scene,copie=original.clone(true),a=original.getObjectByName('volet_mobile')!,b=copie.getObjectByName('volet_mobile')!;
 ...
 const clips=gltf.animations.map((clip,i)=>{
  ...
  const instants=new Set(Array.from({length:17},(_,j)=>j/16*clip.duration));clip.tracks.forEach(p=>Array.from(p.times).forEach(t=>instants.add(t)));
