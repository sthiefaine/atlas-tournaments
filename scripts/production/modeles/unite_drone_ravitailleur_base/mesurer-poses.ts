...
const clips = doc.animations.map(a => {
  ...
  const instants = new Set(Array.from({ length: 385 }, (_, i) => i / 384 * clip.duration));
...
writeFileSync(path.join(sortie, 'mesures-poses-three-glb.json'), JSON.stringify({ id, methode: 'Relecture des positions et pistes depuis le GLB exporté ; Three.js sans texture ni rendu, 385 instants plus clés exactes.', clips, pariteAvantExport: true, approbationArtistique: false }, null, 2) + '\n');
