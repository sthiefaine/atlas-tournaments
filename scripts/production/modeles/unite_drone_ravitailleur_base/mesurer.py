...
identifiant='unite_drone_ravitailleur_base'
...
assert rapport['triangles']<=3500 and rapport['trianglesDegeneres']==0 and rapport['trianglesSuperposes']==0 and rapport['uvTrianglesDegeneres']==0 and rapport['normalesInversees']==0,rapport
...
rapport['rotorsAnimesDansTousClips']=all(all(any(doc['nodes'][c['target']['node']]['name']==nom and c['target']['path']=='rotation' and np.ptp(acc(a['samplers'][c['sampler']]['output']),axis=0).max()>.5 for c in a['channels']) for nom in ['os_rotor_gauche','os_rotor_droit']) for a in doc['animations'])
rapport['osRotorsDansPeau']=len(doc['skins'])==1 and [doc['nodes'][i]['name'] for i in doc['skins'][0]['joints']]==['os_rotor_gauche','os_rotor_droit']
rapport['poidsRotorsRigides']=all(np.all(acc(p['attributes']['WEIGHTS_0'])==np.array([1,0,0,0])) for n in doc['nodes'] if n['name']=='base' for p in doc['meshes'][n['mesh']]['primitives'])
...
assert rapport['noeuds']==['base','corps','module_grue','module_nacelle','os_rotor_droit','os_rotor_gauche','racine','socle'] and len(doc['materials'])==2
...
assert rapport['indicateurCacheFinHorsJeu'] and rapport['rotorsAnimesDansTousClips'] and rapport['osRotorsDansPeau'] and rapport['poidsRotorsRigides']
assert rapport['normalePngLongueurErreurMax']<.008 and rapport['normalePngZMinimum']>0
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'canauxVerifies':True,'rotorsAnimesDansTousClips':rapport['rotorsAnimesDansTousClips']}))
