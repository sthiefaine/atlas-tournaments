...
for a in doc['animations']:
    ...
    instants=sorted(set(instants+list(np.linspace(0,duree,385))))
...
assert np.all(abs(dimensions-np.array([.90,.65,.90]))<np.array([.09,.09,.09]))
rapport={'id':identifiant,'methode':'Relecture GLB NumPy : skin rigide appliqué par jointWorld × inverseBindMatrix, sommets, translations LINEAR et rotations slerp ; 385 instants et toutes clés. Comparaison aux mesures Three.js du GLB exporté à 2 µm près (elles-mêmes recoupées avec la scène source). Majoration continue de toute géométrie sauf témoin par vitesse maximale de translation/slerp et demi-pas. Le témoin rétractable est majoré par sa sphère locale autour du pivot corps immobile horizontalement.','clips':clips,'borneAnalytiqueTemoin':borne_temoin,'bornesStatiques':{'min':mini.tolist(),'max':maxi.tolist(),'dimensions':dimensions.tolist()},'portee':'GLB neutre, gabarit runtime b=[1,1,1] ; PROPORTIONS nationaux a=[.9,.98,1.06] et c=[1.14,1.06,.95] non certifiés ici.','limites':['Contrôle numérique sans rendu ni approbation artistique.','Sol mesuré aux échantillons ; rayon majoré continûment.','Chaque clip lu séparément ; mélanges entre clips non mesurés.','Les collisions internes ne sont pas certifiées par cette mesure d’enveloppe.','Les déformations nationales runtime ne sont pas comprises dans la borne .5 m.']}
