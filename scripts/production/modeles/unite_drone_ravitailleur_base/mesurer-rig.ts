...
async function mesurer(){
 ...
 const rayonInterieurContinu=.171*Math.cos(Math.PI/28),jeux=rayons.map(r=>rayonInterieurContinu-r);
 if(jeux.some(j=>j<=0))throw new Error('Pales hors ouverture facettée');
 const clips=gltf.animations.map(clip=>{
  ...
  const instants=new Set(Array.from({length:385},(_,i)=>i/384*clip.duration));clip.tracks.forEach(p=>Array.from(p.times).forEach(t=>instants.add(t)));
 ...
 const rapport={id,methode:'GLTFLoader natif avec seules références de cartes retirées en mémoire, SkeletonUtils.clone puis AnimationMixer et applyBoneTransform. Aucun rendu.',peauChargee:true,squelettesClonesIndependants:true,centresRotorsFixesDansCorps:true,axes:'+Y',rayonsRotors:rayons,rayonInterieurGardeContinu:rayonInterieurContinu,jeuxMinimauxRotorsGarde:jeux,pariteNumpyDeuxMicrometres:true,clips,limites:['Géométrie neutre uniquement.','Jeu radial ciblé avec le carénage, pas de certification de toutes les collisions internes.','Textures non chargées et aucun rendu.'],approbationArtistique:false};
 writeFileSync(path.join(sortie,'mesures-rig-glb.json'),JSON.stringify(rapport,null,2)+'\n');console.log(JSON.stringify({skinNatif:true,clonesIndependants:true,poses:clips.reduce((n,c)=>n+c.poses,0),jeuxRotors:jeux}));
}
void mesurer();
