...
import numpy as np
identifiant='unite_drone_marin_base'
...
# Seul le fil inox du ressort utilise la cellule d'atlas 3 de cette articulation.
role=np.floor(uv[:,0]*4).astype(int)
indices=indices[np.all(role[indices]==3,axis=1)]
assert len(indices)==288
segments=np.rint((uv[:,0]*4-3-.065)/.87*24).astype(int)
segments=segments[indices].min(axis=1)
triangles=p[indices]
# Le centre de l'âme est le pivot local, rayon 11 mm. Le minimum radial
...
radial[interieur]=0
jeu_ame=float(radial.min()-.011)
assert jeu_ame>0,jeu_ame
# Faces de portions distinctes : écart d'au moins 8 segments (2/3 tour).
# Cela inclut les paires les plus proches entre spires adjacentes et évite
# de nommer "collision" le raccord continu du même morceau de fil.
ia,ib=np.where(np.triu(np.abs(segments[:,None]-segments[None,:])>=8,k=1))
a,b=triangles[ia],triangles[ib]
...
jeu_spires=float(np.min(np.stack(distances)))
assert intersections==0 and jeu_spires>0,(intersections,jeu_spires)
rapport={'id':identifiant,'approbationArtistique':False,'parametresMetres':{'rayonCentral':.022,'diametreFil':.020,'nombreTours':2,'hauteur':.050,'pas':.025,'diametreAme':.022},'glbRelu':True,'trianglesRessort':len(indices),'pairesFacesMesurees':len(ia),'jeuAmeBorneInferieureMetres':jeu_ame,'jeuEntreSpiresMetres':jeu_spires,'intersectionsEntreSpires':intersections,'methode':'GLB exporté : fil identifié par sa cellule UV. Minimum radial continu sur les triangles projetés, moins le rayon de l’âme cylindrique pleine de 11 mm. Distance triangle-triangle (sommets/faces et arêtes/arêtes, avec détection de traversées) entre portions de fil éloignées d’au moins 8 des 24 segments de l’hélice.','limites':['Contrôle local du ressort, pas de certification complète des collisions internes.','Les portions contiguës du même fil sont raccordées intentionnellement.','Le module antenne est une articulation rigide ; ces jeux internes sont invariants dans ses quatre clips.']}
