...
rapport['antenneAnimeeDansTousClips']=all(any(doc['nodes'][c['target']['node']]['name']=='module_antenne' and c['target']['path']=='rotation' and np.ptp(acc(a['samplers'][c['sampler']]['output']),axis=0).max()>.005 for c in a['channels']) for a in doc['animations'])
...
assert rapport['indicateurCacheFinHorsJeu'] and rapport['radarAnimeDansTousClips'] and rapport['antenneAnimeeDansTousClips']
assert rapport['normalePngLongueurErreurMax']<.008 and rapport['normalePngZMinimum']>0
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'canauxVerifies':True,'radarAnimeDansTousClips':rapport['radarAnimeDansTousClips']}))
