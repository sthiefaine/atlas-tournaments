...
rapport['rotorAnimeDansTousClips']=all(any(doc['nodes'][c['target']['node']]['name']=='base' and c['target']['path']=='rotation' and np.ptp(acc(a['samplers'][c['sampler']]['output']),axis=0).max()>.01 for c in a['channels']) for a in doc['animations'])
...
rapport['ressortTours']=2
rapport['ressortPasMetres']=.024
rapport['ressortDiametreFilMetres']=.021
assert rapport['noeuds']==['base','corps','module_antenne','racine','socle'] and len(doc['materials'])==2
assert rapport['materiaux']==['mat_corps','mat_details'] and rapport['quaternionsUnitaires'] and rapport['tempsStrictementCroissants']
assert rapport['indicateurCacheFinHorsJeu'] and rapport['rotorAnimeDansTousClips'] and rapport['antenneAnimeeDansTousClips']
assert rapport['normalePngLongueurErreurMax']<.008 and rapport['normalePngZMinimum']>0
(sortie/'mesures-geometrie.json').write_text(json.dumps(rapport,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'canauxVerifies':True,'rotorAnimeDansTousClips':rapport['rotorAnimeDansTousClips']}))
