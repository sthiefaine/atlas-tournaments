...
def vitesse_limite(canaux):
    ...
    for descendant in points:
        if descendant in [noeuds['socle'],noeuds['base']]:continue  # Borne analytique séparée, indépendante du scale.
        total=0
...
borne_temoin=norme_max_translation(noeuds['socle'])+float(np.linalg.norm(points[noeuds['socle']],axis=1).max())
...
for a in doc['animations']:
    ...
    # Le rotor tourne exclusivement autour de +Y. Son rayon horizontal est invariant.
    # Pour le roulis/tangage du parent, projection horizontale <= rho + |y| sin(theta).
    # Dans la petite boule de rotations (w positif et produits scalaires positifs),
    # SLERP conserve w >= min(w0,w1) : theta est borné par le maximum des clés.
    theta=0.
    for i,canal,temps,valeurs in canaux:
        if i==noeuds['corps'] and canal=='rotation':
            q=valeurs/np.linalg.norm(valeurs,axis=1)[:,None]
            assert np.all(q[:,3]>0) and np.all(np.sum(q[:-1]*q[1:],axis=1)>0)
            theta=max(theta,float(2*np.arccos(np.clip(q[:,3],-1,1)).max()))
        if i==noeuds['corps'] and canal=='translation':assert np.all(valeurs[:,[0,2]]==0)
        if i==noeuds['base'] and canal=='rotation':assert np.all(abs(valeurs[:,[0,2]])<1e-7)
    pbase=np.array(doc['nodes'][noeuds['base']].get('translation',[0,0,0]));assert np.all(pbase[[0,2]]==0)
    rotor=points[noeuds['base']]
    rotor_rho=float(np.linalg.norm(rotor[:,[0,2]],axis=1).max())
    rotor_y=float(abs(rotor[:,1]+pbase[1]).max())
    borne_rotor=rotor_rho+rotor_y*np.sin(theta)
    vitesse=vitesse_limite(canaux);borne=max(borne_temoin,borne_rotor,rayon+max(np.diff(instants))/2*vitesse)
    assert borne<.5 and minimum[1]>-1e-6,(a['name'],borne,minimum)
    clips.append({'nom':a['name'],'nombreEchantillons':len(instants),'enveloppe':{'min':minimum.tolist(),'max':maximum.tolist()},'rayonHorizontalMax':rayon,'vitesseMajoreeMetresSeconde':vitesse,'rayonHorizontalMajorationContinue':borne,'espacementMinimumVoisinsTousCaps':1-2*borne,'racineIdentite':True,'rotorRayonLocal':rotor_rho,'rotorDistanceYMaxAuPivotCorps':rotor_y,'corpsAngleRotationMaxRadians':theta,'rotorMajorationAnalytiqueContinue':borne_rotor})
...
rapport={'id':identifiant,'methode':'Relecture GLB NumPy : sommets, translations LINEAR et rotations slerp ; 193 instants et toutes clés. Comparaison aux mesures Three.js avant export à 2 µm près. Majoration continue du corps/antenne par vitesse maximale et demi-pas ; rotor traité analytiquement par rho + |y| sin(theta) pour toute rotation des pales.','clips':clips,'borneAnalytiqueTemoin':borne_temoin,'bornesStatiques':{'min':mini.tolist(),'max':maxi.tolist(),'dimensions':dimensions.tolist()},'portee':'GLB neutre, gabarit runtime b=[1,1,1] ; PROPORTIONS nationaux a=[.9,.98,1.06] et c=[1.14,1.06,.95] non certifiés ici.','limites':['Contrôle numérique sans rendu ni approbation artistique.','Sol mesuré aux échantillons ; rayon majoré continûment.','Chaque clip lu séparément ; mélanges entre clips non mesurés.','Les collisions internes ne sont pas certifiées par cette mesure d’enveloppe.','Les déformations nationales runtime ne sont pas comprises dans la borne .5 m.']}
