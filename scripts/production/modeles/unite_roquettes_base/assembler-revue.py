#!/usr/bin/env python3
"""Résumé compact lié aux empreintes du lot, des scripts et des rapports."""
from pathlib import Path
from datetime import datetime,timezone
import json,hashlib,sys
ID='unite_roquettes_base';source=Path(__file__).resolve().parent;racine=source.parents[3]
...
# Les sources historiques inspectées doivent être encore présentes et inchangées avant gel.
...
r={'id':ID,'dateRevueUtc':datetime.now(timezone.utc).isoformat(),'statut':'pret_a_integrer_par_coordinateur','approbationArtistique':False,
 'provenance':{'type':'creation_originale_parametrique','scripts':str(source.relative_to(racine)),'sourceExterneUtilisee':False,'ancienCandidatImporte':False,'bakeHD':False,'normales':'Microreliefs analytiques, pigments sans éclairage.','ancienCandidat':{'sha256Glb':ancien['sha256Glb'],'triangles':ancien['triangles'],'octetsGlb':ancien['octetsGlb'],'pngInspectes':len(ancien['textures']),'empreintesReverifieesAvantGel':True},'verificationDistanteCommuniquee':{'date':'2026-09-20T14:56:09.140Z','nombreDepots':0,'fiches':[ID],'origine':'Coordinateur, lecture authentifiée réussie ; revalidation avant intégration à sa charge.'}},
 'sha256Glb':sha_glb,'triangles':geo['triangles'],'budgetTriangles':9000,'primitives':geo['primitives'],'materiaux':geo['materiaux'],'dimensionsMetres':poses['bornesStatiques']['dimensions'],'octetsGlb':fab['octetsGlb'],'octetsLot':sum(f['octets'] for f in fichiers),'fichiers':fichiers,'clips':geo['dureesClipsMs'],
 ...
 'mesuresResume':{'posesParGabarit':sum(c['nombreEchantillons'] for c in poses['clips']),'pariteNumpyThreeNatifToleranceMetres':.000002,'rayonsMajoresABC':rayons,'espaceDeuxVoisinsABC':{g:1-2*r for g,r in rayons.items()},'distancesLocales':len(jeux['distancesTriangles']),'raccordsIntentionnels':len(jeux['raccords']),'raccordsPneusJantes':len(jeux['pneusJantes']),'jeuGardeBoueMinimum':min(c['gardeBoueJeuMinimum'] for c in jeux['posesModules']),'jeuAppuisEntreRoues':min(c['stabilisateursEntreRouesJeuZ'] for c in jeux['posesModules']),'jeuRackCabine':min(c['caissonAuDessusCabineJeuY'] for c in jeux['posesModules'] if c['caissonAuDessusCabineJeuY'] is not None),'sondesCavites':sum(c['rayons'] for c in jeux['cavites']),'sondesTir':sum(c['rayonsLibres'] for c in jeux['passagesTir']),'racineNonAnimee':not geo['racineAnimee'],'tempsAnimationsMinMaxConserves':geo['bornesTempsAnimationConservees'],'bouclesPoseEtVitesse':True,'horsJeuTientPoseEtMasqueTemoin':geo['horsJeuTientPoseFinale'] and geo['indicateurCacheFinHorsJeu']},
 'correctionsAvantGel':['Extrémités resserrées pour le gabarit c.','Roues placées selon le rayon circonscrit des crampons.','Vitrages alignés sur les plans de cabine.','Patins raccordés aux rails et attaches de garde-boue relevées.'],
 'limites':['Aucun rendu, capture, contrôle visuel ni approbation artistique.','Aucun test général, build, téléphone ou FPS.','Clips isolés ; transitions et mélanges exclus.','Sol et jeux aux poses, rayon majoré continûment par nœud pour a/b/c.','Distances locales et sondes ponctuelles ; aucune certification globale des collisions.','Vitrages opaques sans intérieur ; atlas répété ; suspension simplifiée.','21 primitives articulées, coût réel mobile non mesuré.','Aucun HD ni bake ; lisibilité et effets de bouche à apprécier humainement.'],
