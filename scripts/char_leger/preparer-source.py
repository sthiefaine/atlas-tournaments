...
SRC=Path(sys.argv[1]); ID='unite_char_leger_base'; OUT=ROOT/'assets/livraisons'/ID; OUT.mkdir(parents=True,exist_ok=True)
b=SRC.read_bytes(); sha=hashlib.sha256(b).hexdigest()
if sha!='82f0857a3f0074cbd2aca688ec81c2227f53fa1a1dcbba66d6579f399a513f99': raise ValueError('Source différente de la révision inspectée')
...
for start in range(0,len(idx),3):
 ...
 groupe=0 if y<.15 else 2 if y>.26 else 1
...
nodes=[{'name':'racine','children':[1,2]},{'name':'base','mesh':0},{'name':'corps','translation':[0,.16,0],'mesh':1,'children':[3]},{'name':'module_tourelle','translation':[0,.105,0],'mesh':2,'children':[4]},{'name':'socle','translation':[0,.055,.178],'mesh':3}]
animations=[]
for name,dur in [('repos',2.4),('deplacement',1),('tir',.7),('touche',.5),('hors_jeu',.9)]:
 ...
 if name=='repos': track(3,'rotation',[(0,math.sin(a/2),0,math.cos(a/2))for a in [0,.25,0,-.25,0]],4)
 ...
 if name=='hors_jeu': track(4,'scale',[(v,v,v)for v in [1,1,.5,0,0]],3)
