...
export function vegetationPlaine(lod:0|1):T.BufferGeometry{
  ...
  for(let i=0;i<240;i++){
    ...
    const x=((i%16)+.22+hasard()*.56)/16-.5,z=(Math.floor(i/16)+.22+hasard()*.56)/15-.5;
    const a=hasard()*Math.PI*2,h=i===0?.04:.021+hasard()*.019,w=.012+hasard()*.011,courbe=.012+hasard()*.018;
    const teinte=pigments[Math.floor(hasard()*pigments.length)]!;
    if(lod===1&&i%5!==0)continue;
    const points=[[-w/2,0,0],[w/2,0,0],[-w*.3,h*.57,courbe*.35],[w*.3,h*.57,courbe*.35],[0,h,courbe]];
    ...
    for(const ids of [[0,1,2],[1,3,2],[2,3,4]])triangle(ids.map(n=>monde[n]!),teinte);
  }
  // Six minuscules trèfles : feuilles inclinées, aucune grosse fleur qui marque chaque répétition.
  for(let i=0;i<6;i++){
