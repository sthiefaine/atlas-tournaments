...
SRC=Path(sys.argv[1]); ID='batiment_qg_base'; OUT=ROOT/'assets/livraisons'/ID; OUT.mkdir(parents=True,exist_ok=True)
b=SRC.read_bytes(); sha=hashlib.sha256(b).hexdigest()
if sha!='71a2d4e7aeab754dfa8edc0e5708d5021e4f4bd88204654a8be20eeb168bb326': raise ValueError('Source différente de la révision inspectée')
...
# Axes source conservés ; mise aux dimensions et normales inverse-transposées.
sx=sy=sz=.80/(hi[0]-lo[0])
positions=[(sx*(x-(lo[0]+hi[0])/2),sy*(y-lo[1])+.025,sz*(z-(lo[2]+hi[2])/2)-.06)for x,y,z in pos]
normals=norm
pivots=[(0,0,0),(0,0,0),(0,.82,.30)]
groupes=[[],[],[]]
for start in range(0,len(idx),3):
 ids=idx[start:start+3];y=sum(positions[j][1]for j in ids)/3
 groupes[1 if y>.47 else 0].extend(ids)
...
albedo.save(OUT/f'{ID}_albedo.png');normal.save(OUT/f'{ID}_normale.png');rm.save(OUT/f'{ID}_rugosite.png');mask.save(OUT/f'{ID}_masque_equipe.png')
Image.new('RGB',(512,512)).save(OUT/f'{ID}_emission.png')
...
# Ajouts de parcelle, mât et fanion ; aucune modification des triangles source.
white=next((i for i,v in enumerate(mask.getdata())if v==255),None)
if white is None: raise ValueError('Masque équipe vide')
teamuv=((white%512+.5)/512,(white//512+.5)/512)
neutral=next(i for i,v in enumerate(mask.getdata())if v==0)
neutraluv=((neutral%512+.5)/512,(neutral//512+.5)/512)
def box(centre,size,group,tex):
 for axis in range(3):
  for sign in [-1,1]:
   a=(axis+1)%3;c=(axis+2)%3;start=len(positions);corners=[(-1,-1),(1,-1),(1,1),(-1,1)]
   if sign<0:corners.reverse()
   for aa,cc in corners:
    v=list(centre);v[axis]+=sign*size[axis]/2;v[a]+=aa*size[a]/2;v[c]+=cc*size[c]/2;positions.append(tuple(v));nn=[0,0,0];nn[axis]=sign;normals.append(tuple(nn));uv.append(tex)
   groupes[group].extend([start,start+1,start+2,start,start+2,start+3])
box((0,.0125,0),(.85,.025,.85),0,neutraluv)
box((.32,.475,.30),(.025,.95,.025),0,neutraluv)
box((.24,.88,.30),(.16,.10,.02),2,teamuv)
meshes=[]
...
nodes=[{'name':'racine','children':[1]},{'name':'corps','mesh':0,'children':[2,3]},{'name':'toit','mesh':1},{'name':'enseigne','translation':[0,.82,.30],'mesh':2}]
animations=[]
for name,dur in [('repos',3.2),('capture',1.4),('touche',.4)]:
 t=add([(dur*i/4,)for i in range(5)],1,bounds=True)
 angles=[0,.035,0,-.035,0] if name=='repos' else [0,.15,0,-.08,0] if name=='capture' else [0,-.1,.04,0,0]
 output=add([(0,math.sin(a/2),0,math.cos(a/2))for a in angles],4)
 animations.append({'name':name,'extras':{'loop':name=='repos'},'samplers':[{'input':t,'output':output,'interpolation':'LINEAR'}],'channels':[{'sampler':0,'target':{'node':3,'path':'rotation'}}]})
...
report={'source':SRC.name,'sha256':sha,'trianglesSource':len(idx)//3,'trianglesLivres':sum(len(x)//3 for x in groupes),'trianglesAjoutes':36,'trianglesParAttache':[len(x)//3 for x in groupes],'pixelsEquipe':sum(x==255 for x in mask.getdata()),'approbationArtistique':False,'limites':['Segmentation géométrique des attaches à vérifier humainement','Absence de lumière cuite non certifiée','Orientation front à confirmer dans inspecteur','Émission noire conservatrice : fenêtres source non isolées','Source mise à échelle uniforme ; parcelle et fanion ajoutés pour gabarit']}
