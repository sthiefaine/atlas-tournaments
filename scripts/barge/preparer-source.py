...
normals=[]
for x,y,z in norm:
 v=(-z/sz,y/sy,x/sx);length=math.sqrt(sum(a*a for a in v));normals.append(tuple(round(a/length*32767)for a in v))
attrs={'POSITION':add(positions,'f',3,5126,bounds=True),'NORMAL':add(normals,'h',3,5122,True),'TEXCOORD_0':add([(u,.125+.875*v)for u,v in uv],'f',2,5126)}
parts=[[],[]]
...
doc={'asset':{'version':'2.0'},'buffers':[{'byteLength':len(out)}],'bufferViews':views,'accessors':accessors,'meshes':meshes,'nodes':[{'name':'base','mesh':0},{'name':'module_grue','mesh':1}],'scenes':[{'nodes':[0,1]}],'scene':0,'extensionsUsed':['KHR_mesh_quantization'],'extensionsRequired':['KHR_mesh_quantization']}
