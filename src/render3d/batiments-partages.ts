...
import { clonerFigurine, clonerMateriauNoeud, definirMasque, lectureDepuisGltf, libererSquelettesPrives, masqueDe } from './modeles';
...
export async function acquerirBatiment(id: string): Promise<THREE.Object3D> {
  ...
  if(!entree) {
    entree = {refs:0,lecture:new GLTFLoader().setMeshoptDecoder(MeshoptDecoder).loadAsync(`/assets/modeles/${id}_lod0.glb`).then(lectureDepuisGltf).then(l=>{
      // Object3D.clone partage les clips, mais chaque case possède son lecteur.
      l.scene.animations = [...l.clips];
      return l.scene;
    })};
  ...
  try {
    ...
    copie.name=id;
    baux.set(copie,()=>{libererSquelettesPrives(copie);for(const m of clones.values())m.dispose();rendre();});
    return copie;
