...
export default function Depart(): React.ReactElement {
  ...
  return <main className={styles.page}><div className={styles.fond} aria-hidden="true" /><section className={styles.panneau}>
    ...
    {etape === 'mode' && <><div className={styles.choix}><button className={styles.carte} onClick={() => partir('normal')}><span className={styles.insigne}>N</span><strong>Normal</strong><span>Découvrez l’histoire, préparez vos attaques et faites vos choix.</span><b>Ouvrir ma campagne →</b></button><button className={styles.carte} disabled={!profil || !debloques[profil]} aria-describedby="condition-difficile" onClick={() => partir('difficile')}><span className={styles.insigne}>{profil && debloques[profil] ? 'D' : '🔒'}</span><strong>Difficile</strong><span>Des adversaires mieux équipés. Chaque placement compte.</span><b>{profil && debloques[profil] ? 'Ouvrir ma campagne →' : 'Verrouillé'}</b></button></div><p id="condition-difficile" className={styles.note}>Remportez la finale de la campagne en mode normal avec ce profil pour débloquer Difficile. Les tutoriels et la saison 1 seule ne suffisent pas.</p></>}
