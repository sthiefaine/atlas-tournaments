...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="admin-shell">
    ...
    <header className="admin-entete"><div><Link href="/" className="admin-marque">Atlas Tournament</Link><h1>Atelier de création</h1></div>{session ? <form action="/api/admin/session?methode=fermer" method="post"><button type="submit">Déconnexion</button></form> : null}</header>
