...
export function specUnite(u: UnitType): AssetSpec {
  ...
  return {
    ...
    budget: ['barge', 'artillerie', 'infanterie', 'antiair'].includes(u.cle) ? budget(1000000, u.cle === 'infanterie' ? 2 : 3) : budgetUnite(s),
    textures: ['barge', 'artillerie', 'infanterie', 'antiair'].includes(u.cle) ? textures.map(t => (['albedo', 'normale'].includes(t.canal) || (['artillerie', 'infanterie', 'antiair'].includes(u.cle) && ['rugosite', 'metal'].includes(t.canal))) ? { ...t, resolution: 4096 as const } : t) : textures,
