...
test('barge active : chargement des trois LOD et des clips dans la vitrine',async({page})=>{
 ...
 for(const lod of [0,1,2]){await expect.poll(()=>recus.has(`/assets/modeles/unite_barge_base_lod${lod}.glb`)).toBe(true);await page.getByRole('combobox',{name:'Niveau de détail',exact:true}).selectOption(String(lod));}
