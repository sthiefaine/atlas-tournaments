...
Les prompts de cette file précisent l'autorisation de création originale lorsque le stockage a été lu avec succès sans dépôt et qu'aucun maître local n'existe. Un échec de stockage bloque la préparation. Le prompt générique de l'admin continue à privilégier le GLB uploadé ; son instruction de réclamer une source absente est adaptée uniquement dans cette commande globale. Le spécialiste reste dans son dossier de scripts et son staging, puis gèle les fichiers ; l'intégration et Git appartiennent au coordinateur. La plaine porte une exclusion explicite.
...
**Depuis le 23 septembre 2026, le jeu n'affiche plus les GLB en temps réel** : la peau 3D est retirée, et les modèles sont photographiés hors ligne en images cuites (`npm run cuire:sprites`, `doc/18-rendu-sprites.md`), qui lit `public/assets/modeles/`. Une livraison active se voit donc en jeu après sa cuisson. Les deux paragraphes qui suivent décrivent le chargement par la peau 3D, gardés pour l'histoire des livraisons ; les clips et les marqueurs qu'ils citent restent dans les fichiers, et c'est la cuisson qui les lit désormais.
...
Les unités et bâtiments actifs sont chargés automatiquement. Les rochers actifs sont sélectionnés par biome, chargés par le même cache de GLB, puis fusionnés pour les lots instanciés des sommets et des grèves. `rochers-livres.ts` préserve les UV et normales, adapte l'encombrement au semis existant et retire les tangentes : les lots transforment les normales, et Three reconstruit ainsi le repère tangent à partir des positions et UV transformés. Chaque décor possède ses matériaux et sa géométrie fusionnée ; les textures restent partagées jusqu'au dernier bail. Le repli procédural demeure si le modèle manque ou n'est pas compatible. Une marée refait le semis sans recharger le GLB. Un chargement interrompu rend les ressources qui n'ont pas encore été transmises au monde.

Le relevé `mesurer-rocher-runtime.ts`, qui mesurait un rocher dans les lots instanciés de la peau 3D, est retiré avec elle (23 septembre 2026) : ces lots n'existent plus. Les terrains fournissaient leurs textures au relief de la peau 3D, sans chargement général de leur maillage. Les portraits de commandants sont encore vectoriels : leur raccordement reste à traiter lors de leurs livraisons, un alias publié ne suffit pas à annoncer leur affichage en jeu.
