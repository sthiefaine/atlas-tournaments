...
Shared, unmarked tournament vehicle. Three self-contained glTF 2.0 binary models and six separate PNG channels are in the delivery ZIP and `public/assets/modeles/`.

- LOD0: 2,704 triangles; LOD1: 1,712; LOD2: 588.
...
- External roughness and metal maps are separate grayscale images. Each GLB embeds a derived metallic-roughness map (G = roughness, B = metal) for standard PBR readers, plus all six channel images. No external fetch is necessary for standard rendering. Team-mask texture index is stored in material/document extras for the game's custom recolouring.
...
`atlas.json` records the stable 16 × 16 part addresses. Each 512 px atlas cell has a four-pixel guard band. All LODs use the same texture files and logical part addresses. The atlas should be painted in place for national kits.
