...
Set units to metres, +Y up and +Z front. Place the origin at the footprint centre at ground level. Apply transforms and verify world-space bounds including protrusions. Create exactly the node names, material names and required LOD files specified for this asset. Do not add export helper nodes or a ground plane. Preserve the hierarchy and UV registration across the required LODs and national material kits. Reuse the approved base mesh for national kits instead of reconstructing variants from images.
