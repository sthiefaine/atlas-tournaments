...
**Mise à jour du 7 septembre 2026.** Le catalogue 5 (neuf unités d'air et de mer, le `port` sixième bâtiment capturable) portait le compte à **947** ; le catalogue 6 (le chasseur furtif, 24ᵉ et dernière place du plafond) le porte à **972** : 24 `unite`, **576 kits** (24 × 24), 8 terrains, **246 bâtiments** (6 capturables × 41 territoires), 108 décors, 10 commandants — **261** en priorité 1, 173 en 2, 538 en 3. Toujours aucune livraison hors l'infanterie générée par script ; `--verifier` fait foi.
...
Le char léger candidat utilise `EXT_meshopt_compression`, requis sans fallback de données non compressées. Les codecs ATTRIBUTES et INDICES conservent les données sans quantification, sans réordonnancement ni décimation. PNG externes inchangés. Les chargeurs du jeu, de l’environnement et de l’inspecteur utilisent le décodeur Meshopt WASM. Le contrôle binaire et les empreintes de géométrie partagée décompressent avant d’examiner les données. Le contrôle synchrone utilise le décodeur JavaScript de référence (plus lent, réservé à la réception) ; ses allocations sont bornées à 96 Mio et les formats/offsets sont contrôlés avant décompression. Les fichiers non compressés gardent leur chemin de lecture existant.

Commande de production : `node --import tsx scripts/production/compresser-source.ts unite_char_leger_base`. La première portée est volontairement ce candidat. Les PNG et la source privée ne sont pas recompressés avec perte. La présence de l’extension ne vaut pas validation : pour cette livraison, les tests et le contrôle final n’ont pas été relancés à la demande du propriétaire. La version de réception est signalée non contrôlée après compression ; le verdict antérieur ne lui est pas attribué.
