...
#### Les familles de la faction — 10 septembre 2026

Le propriétaire voulait des supers de vilains qui ne soient pas « −1 PV à toute l'armée » : « des missiles qui font −3 à certaines zones de la carte, surtout là où il n'y a pas ses unités ; des rayons laser qui font −X à certaines de mes unités ; des IEM qui m'empêchent de bouger mes unités à moteur pendant un tour et me font perdre les avions et drones touchés ». Trois familles de plus, **réservées à la faction** — et à elle seule : un commandant national ne les porte jamais, une routine ne peut pas les lui donner.

| Famille | Forme et bornes | Au pouvoir normal | Ce que le moteur en fait | Événement |
|---|---|---|---|---|
| **`frappe`** de zone | `{ cible: 'terrain', frappe: { pv: 1..3, rayon: 0..2 } }`, **`cases[0]` obligatoire** (refus `pouvoir_invalide` sans case ou hors carte, avant que la jauge soit payée) | `pv` ≤ 2 et `rayon` ≤ 1 | toute unité à `rayon` pas Manhattan du centre — **des deux camps**, un transport compris, sa cale non — perd `pv` PV affichés, plancher 1 PV interne | `frappe_zone { camp, centre, rayon, touchees: [{ uniteId, pv }] }` |
| **`laser`** | `{ cible: 'unites_adverses', laser: { pv: 1..5, nombre: 1..3, choix: 'plus_cheres' \| 'plus_avancees' } }`, sans case | libre | les `nombre` unités adverses que le camp **voit** (brouillard honnête : `adversesVisibles`), les plus chères — à égalité l'identifiant le plus petit — ou les plus proches de son QG (à défaut, du barycentre de ses unités), perdent `pv` PV affichés, plancher 1 PV | `rayon_laser { camp, touchees }` |
| **`iem`** | `{ cible: 'terrain', iem: { rayon: 1..3, abattre } }`, **`cases[0]` obligatoire** | `abattre: false` seulement | toute unité **à moteur** (`TYPES_MOUVEMENT_MOTEUR` : roues, chenilles, air, mer, amphibie) dans le rayon, **des deux camps**, reçoit l'arrêt de l'impulsion de station — `iemJusquaJournee`, `etat = 'agi'`, ni mouvement ni riposte jusqu'à la fermeture de son prochain tour, sans dégât ; si `abattre`, les unités **adverses** du domaine `air` touchées, drones compris, sont **mises hors jeu** par `mettreHorsJeu` (cale comprise, `hors_jeu` par unité, jauge du camp abattu créditée du subi, rien au déclencheur) | `iem_pouvoir { camp, centre, rayon, immobilisees, abattues }` |

**« Ce camp est la faction »** a une seule source de vérité : `reglages.factionsParCamp[camp] === 'atl'`, posé par `Scenario.factionsParCamp` (`etat.ts`), celui que `catalogue.ts` lit déjà pour les prototypes `factionExclusive`. Le **schéma** (`validerCommander`) refuse les trois familles à un commandant dont le `paysCode` n'est pas `atl` — dans `commandants-capacites.json`, un commandant de la faction porte `faction: "atl"` et pas de `paysCode`, et c'est sous `atl` que le test le présente au validateur — ; le **moteur** (`verifierPouvoir`) les refuse `pouvoir_invalide` à un camp qui n'est pas la faction, ceinture et bretelles. Le schéma refuse aussi au pouvoir normal une frappe au-delà de 2 PV ou d'une case, et `abattre` hors d'un super d'au moins **8 barres** ; le moteur redit les deux.

**`iem.abattre` est une mise hors jeu directe du jeu**, décision du propriétaire : l'interdit « jamais de mise hors jeu directe » du §7.2 reste **entier pour tout commandant national**, avec les exceptions nommées du super de la faction et de la station renforcée de scénario (12 septembre 2026). Les trois familles sont instantanées et ne posent rien dans `etat.modificateurs` ; `evaluerEffets(…, cases)` en donne le bilan à case donnée — `touchees`, `immobilisees`, `abattues` — pour le HUD et pour l'IA. Aucune ne crédite la jauge du déclencheur, pas plus que `degats_directs`.

**L'IA vise** (`src/ai/pouvoirs.ts`, `meilleureCase`) : pour une frappe ou une impulsion, elle parcourt toutes les cases de la carte et garde celle qui maximise la valeur adverse touchée moins la valeur propre touchée, en fonds — PV retirés × coût/10 pour une frappe ; pour une impulsion, un tour perdu vaut un dixième de la valeur de l'unité, plafonné (`PLAFOND_TOUR_PERDU` 1 500), une unité mienne qui a déjà joué ne coûte rien, une abattue vaut son coût entier. Une case qui toucherait plus de sien que d'adverse vaut zéro : le pouvoir ne part pas. L'action envoyée au moteur porte `cases: [meilleure]` (`actionPouvoir`). Le laser n'a pas de case : sa valeur se lit comme des dégâts directs. Tests : `tests/engine/pouvoirs-faction.test.ts`, `tests/ai/pouvoirs-faction.test.ts`.
...
### 7.7 Superusine, automate, IEM sur les usines, radar — 10 septembre 2026

Quatre demandes du propriétaire pour le dernier chapitre : « des cartes où les ennemis ont une superusine qui produit chaque tour une unité robot », « une mission vraiment complexe et dure où la condition de victoire est l'élimination totale », « les IEM retardent aussi la production dans nos usines », « des radars capturables qui aident ». Ce qui suit est câblé dans le moteur et l'IA ; l'écran en fait ce qu'il veut.

**La superusine** (`Scenario.superusines`, `Superusine` dans `03-schemas`). Un bâtiment **producteur** — usine, aéroport, port — ou le **QG** d'un camp, désigné par le scénario avec un type d'unité, une première journée (`depuisJournee`, 1 à défaut), une période (`chaque`, 1 à défaut) et un plafond (`max`, illimité à défaut). Au **début du tour** de ce camp, phase 5 après le réveil, quand la journée est due, une unité neuve paraît **sur la case** si elle est libre, sinon sur **la première voisine libre et franchissable** dans l'ordre fixe de `voisines` (nord, ouest, est, sud) ; s'il n'y en a aucune, la production est sautée sans report ni compte. L'unité arrive `prete`, à 100 PV, au plein de munitions et de carburant, et **ne coûte rien** : c'est **la seule production gratuite du jeu**, une exception nommée de scénario — l'interdit « jamais de production gratuite » du §7.2 reste entier pour tout pouvoir et toute mécanique. Événement `production_automatique { camp, uniteId, unite, case }`, compteur `etat.superusinesProduites[rang]`. **Elle se prend comme une usine** : dès que la case n'est plus au camp — capturée par un adversaire, ou désaffectée —, rien n'en sort, et rien n'a à être marqué dans l'état ; rendue à son camp, elle repart. Un génie ne peut pas la « mettre hors service » puisque rien ne désaffecte un bâtiment en service (§6 bis) : la seule prise est la capture, quarante points sur un QG, vingt ailleurs. **On peut la capturer, mais on ne peut rien en faire** (10 septembre 2026 au soir, décision du propriétaire) : `verifierProduction` refuse **`usine_inerte`** sur une case qui porte une superusine du scénario, pour le camp qui la tient comme pour son camp d'origine — celui-ci n'a que la production automatique, jamais le menu. `producteursDe` ne la compte pas (l'IA n'y achète rien, `usinesLibres` la lit au moteur, une impulsion IEM ne la marque pas), et le revenu, lui, reste celui de tout bâtiment tenu : `revenuParTour` compte les bâtiments, pas les producteurs. `validerScenario` vérifie la forme (case dans la carte, camp connu, `chaque ≥ 1`, `max ≥ 1`), `creerPartie` la parole : bâtiment producteur ou QG **du camp**, type du catalogue, exclusivité respectée. L'IA joue les unités produites comme les siennes.

**L'automate de combat méridien** (`meridien_automate`, catalogue **9**, 10 septembre 2026), troisième matériel exclusif des Gris après le Veilleur et le Bastion : chenilles, mouvement 5, vision 2, carburant 60, **9 000**, **tir illimité** (`munitions: null`), riposte, silhouette `chenilles · bloc · [tourelle, canon_long, antenne] · 2`. Bon contre le mou et les véhicules légers (60 à 75), moyen contre les chars (35 à 45), **rien en l'air** ; **touché par tout ce qui touche un char moyen** — sa colonne est celle du char moyen recopiée, la contrainte 2 du §13.3 tient par le char lourd et le lance-roquettes à 70. Trente unités au catalogue : le plafond du validateur passe de 29 à 30, celui du §13.7 est dépassé depuis le catalogue 7 et se lit comme un plafond **par catalogue joué**, pas du canon. `chargerCatalogue(8)` ne le voit pas ; aucun kit national n'est commandé, comme pour les deux autres exclusives.

**L'IEM retarde la production.** Une impulsion — de station (`installationsIem`) comme du pouvoir `iem` de la faction — touche aussi, dans son rayon, **tout bâtiment producteur** (`producteursDe` : usine, aéroport, port, QG) d'un camp **adverse** — ni le camp, ni un allié, ni un camp éliminé. `etat.usinesIem[case] = journee` ; `verifierProduction` refuse **`usine_iem`** tant que l'entrée est là, et l'entrée est levée **à la fermeture du tour de son propriétaire**, exactement comme `Unite.iemJusquaJournee` — une usine touchée perd donc un tour de production, ni plus ni moins, que l'impulsion soit tombée avant ou après son tour dans la journée. Un événement `usine_iem { camp, case }` par bâtiment ; `iem_pouvoir.usines` et l'annonce de station portent le compte. L'IA (`valeurCase`, `detailPouvoir`) compte une usine adverse touchée comme la part de revenu d'un tour qui revient à chaque producteur du camp, bornée à `PLAFOND_USINE_IEM` (1 500). `VERSION_MOTEUR` passe à **8**.

**Le radar ne télégraphie pas** (retiré le 10 septembre 2026 au soir, sur décision du propriétaire). Une version du matin faisait rapporter par la station radar ce qu'un camp adverse déclencherait maintenant (`intentionSuper`, cases marquées sur la carte) ; elle est retirée entière — IA, page, rendu, chaînes. La station radar reste exactement ce qu'elle était (§10 bis) : **vision 5 et brouillage 12**, lus au moteur (`VISION_STATION_RADAR`, `RAYON_STATION_RADAR`), et c'est tout ce que le panneau de case en dit.
...
### Station renforcée de scénario — 12 septembre 2026

`installationsIem[].mode: "renforcee"` abat le domaine air adverse (hélicoptères et drones compris, cargaison comprise) avec les événements `hors_jeu`, le crédit de jauge subi et l'impulsion visuelle existants. Le domaine mer reste immobilisé un tour ; les producteurs adverses sont arrêtés comme sous l'IEM classique. Pas de nouvelle animation de chute physique.

Version locale : rayon 2 par défaut, borné 1–3. Version mondiale : `portee: "carte"` et `campProtege` obligatoire, sans rayon. Le camp protégé et ses alliés sont immunisés ; une capture par un autre camp désactive la station au lieu de retourner l'arme. Une station désaffectée ou neutre est éteinte. Première impulsion ≥ J6, intervalle 6–10 (6 par défaut). L'annonce publique indique le temps restant dès J1. Les réglages sans `mode` conservent exactement le comportement classique. Aucun scénario public n'active cette extension dans cette livraison.
