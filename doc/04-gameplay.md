...
**`iem.abattre` est la seule mise hors jeu directe du jeu**, décision du propriétaire : l'interdit « jamais de mise hors jeu directe » du §7.2 reste **entier pour tout commandant national**, et n'a que cette exception, nommée, au super de la faction. Les trois familles sont instantanées et ne posent rien dans `etat.modificateurs` ; `evaluerEffets(…, cases)` en donne le bilan à case donnée — `touchees`, `immobilisees`, `abattues` — pour le HUD et pour l'IA. Aucune ne crédite la jauge du déclencheur, pas plus que `degats_directs`.
