...
**État au 6 septembre 2026 (soir)** — A1 à A6 sont **faits**, le lot A est clos et **J1 est acquis** ; le détail est dans `10-rendu-3d.md` §6.5, §9.2 et §9.3.
...
- **A3, fait.** `EffectComposer` (`RenderPass` → `GTAOPass` → `OutputPass` → grain) dans `scene.ts`, chargé par `import()` à l'activation, boucle paresseuse conservée, `capturer()` lit l'image composée, réglage « Qualité d'affichage » à trois choix dans `/reglages`, `auto` mesuré sur les dix premières images (seuil 20 ms, GPU compris — 8 au premier réglage, porté à 20 après la mesure A6), jamais sous le rasteriseur logiciel. *Reste* : le seuil et les paramètres de l'occlusion (rayon 0,35 case, résolution logique) sont raisonnés, pas mesurés ; la fumée Playwright doit être relancée après fusion.
...
- **A5, fait.** Les sept rôles de `Materiaux` sont des matières réglées avec le studio — tôle peinte à émission presque nulle, acier franc, verre translucide qui n'ombre pas, caoutchouc, peau mate —, la pluie les mouille par jeu de matériaux, et les toits reçoivent un micro-relief synthétisé (tuile, ardoise ou tôle ondulée d'après le style régional, `jeuToit`), cartographié à l'échelle du monde ; le détail est dans `10-rendu-3d.md` §5 et §7.4. *Reste* : les valeurs sont raisonnées et bornées par des tests, pas jugées à l'œil ; la capture de référence du banc a remplacé `doc/assets/render-3d.png` avec A6, ci-dessous.
- **A6, fait** (6 septembre 2026, nuit). La table mesurée est au §9.2 de `10-rendu-3d.md`, avec la méthode chiffre par chiffre et la table des ordres de grandeur gardée au-dessus pour la comparaison. L'instrument : `Rendu.mesurer()` rend désormais le **détail par famille** (`render3d/mesures.ts`, pur, testé sur des groupes construits en mémoire ; `familles` dans `MesuresRendu`), le banc porte une **grande carte 24 × 16** (`carteGrande`, monde 4 : générée par `mapgen` avec une graine fixe — 31 % de forêt, 16 montagnes, 32 cases de mer, 22 bâtiments possédés, 30 unités des quatorze types, l'état identique d'une construction à l'autre, testé) et son pont expose `jouer(geste)`, `neuf()`, `capturer()` et `feuille(n)` ; le pilotage, lui, est un script Playwright hors dépôt. Chiffres clés, Apple M1 par ANGLE Metal, 1280 × 800, grande carte : **381 620 triangles et 877 appels** l'image nue — la scène seule en fait 210 576 et 526, le reste est la passe d'ombres —, 576 466 et 1 364 composée ; calibration `auto` de 14,5 ms ; en animation, **60 images par seconde** sans chaîne (17,3 ms entre deux images, borné par la synchronisation verticale) et 25 avec (40,1 ms). Les triangles sont donc **au budget** (210 000 visés), les appels **vingt fois au-dessus** (27 visés), et 504 des 526 tirages viennent des unités (onze mailles par figurine : sept rôles, socle, repères, étiquette) et des bâtiments (sept mailles par case, une par matériau) — les deux familles que `10` §9.1 propose d'instancier et qui ne le sont pas ; rien n'y est fait, c'est la passe de performance du lot B. Autre constat : la calibration mesure 12 à 18 ms sur ce M1 selon la carte et la fenêtre, au-dessus du seuil de 8 ms d'alors — en `auto`, **la chaîne ne s'allumait sur aucune carte ici** ; le seuil a été porté à 20 ms sur cette mesure, elle s'allume désormais partout sur ce M1 à 25–36 images par seconde en animation, et la cadence lui donne raison, puisqu'elle coûte le double annoncé. Le seuil tient. Sous SwiftShader — ce que le test de fumée subit —, l'image nue coûte 0,9 à 1,4 s et l'image composée 1,5 à 2,9 s (médianes en animation), la calibration 0,8 à 1,3 s : la chaîne n'y est jamais allumée en `auto`, comme la fumée l'exige ; deux relevés logiciels sont des artefacts de la machine partagée et sont nommés tels dans la table. *Reste* : **un téléphone réel n'a pas été mesuré** — la fenêtre de 390 × 844 à ratio 2 est le même M1 avec des ombres 1024² et une toile étroite, rien d'autre — ; les relevés viennent d'une machine partagée avec d'autres tests, en Chromium headless ; la démo n'a pas de cadence en animation, faute de geste rejouable ; et le coût des `SkinnedMesh` du lot C (§4, point 4) n'est pas mesurable, aucun n'existant.
- **J1, acquis.** Les trois captures de référence du banc — `doc/assets/banc-ete-clair.jpg`, `banc-automne-pluie.jpg`, `banc-hiver-nuit.jpg` : Metal, 1280 × 800, chaîne allumée, dock replié, lues sur la toile seule par `__atlasBanc.capturer()` puis converties en JPEG par `sips`, à la qualité **75** plutôt que 85 parce que le grain de la chaîne portait les trois au-dessus de 300 ko (365 à 377 ko à 85, 271 à 283 à 75) — et `render-3d.jpg`, qui est la première d'entre elles et remplace `render-3d.png` ; la table du §9.2 remplie de chiffres réels ; la fumée Playwright verte depuis la fusion. **Personne n'a relu ces captures** : ce sont des documents pour le propriétaire, pas une vérification.
...
La vitrine porte le badge « Modèle livré / Placeholder », le choix du niveau et le lecteur de clips, et `window.__atlasVitrine.modele()` les expose au pilotage. Rien de tout cela n'avait vu un vrai fichier à l'heure où ces lignes ont été écrites ; le premier est arrivé le soir même, ci-dessous.

**La première base livrée est l'infanterie, générée par script (6 septembre 2026, soir).** `public/assets/modeles/` n'est plus vide : `unite_infanterie_base_lod0.glb`, `_lod1`, `_lod2` et quatre PNG — `albedo` et `normale` en 1024, `rugosite` (le métal dans son canal bleu, c'est la carte `metallicRoughness` de glTF) et `masque_equipe` en 512 —, produits par `npm run generer:infanterie` (`scripts/generer-infanterie.ts`, modules dans `scripts/infanterie/` : atlas UV, maillage, figurine, clips, textures, écriture glTF). Une escouade de trois fusiliers à cinq têtes et demie, chacun sur ses onze os — bassin, colonne, tête, bras et avant-bras, cuisse et jambe, des deux côtés — sous une ancre, trente-sept os en tout avec la racine `base` ; un seul maillage à deux matériaux, `mat_corps` sous le masque (maillot, casque, sac : la palette de la nation) et `mat_details` hors masque (trois visages peints, trois teints, mains, bottes, lanceur de marqueurs à embout orange, socle) ; les six clips de la spécification, échantillonnés depuis des fonctions de pose déphasées d'une figurine à l'autre, chaque os ayant une piste dans chaque clip pour que les fondus du mixer n'aient rien à deviner ; trois niveaux par **finesse** — moins de segments, moins de pièces — et non par décimation : 3 918, 1 368 et 396 triangles pour 4 000, 1 400 et 400 ; 737 ko en tout. L'écriture passe par `GLTFExporter` sous Node avec une seule cale (`FileReader`), puis un dédoublonnage des accesseurs — l'exportateur en écrit un par piste, cent kilo-octets de JSON pour rien — et l'injection des cartes par `uri` voisine, le chargeur three lisant alors les fichiers d'à côté.

Ce que cela prouve, et ce que cela ne prouve pas. La boucle B1 a tourné de bout en bout — production, conformation, contrôle (`controler:asset` rend `ok` sur les trois niveaux, textures voisines comprises), dépôt — et le placeholder a disparu de la vitrine et du jeu **par le seul inventaire** : la règle « un changement de fichier, pas une ligne de code » exercée pour de vrai, et `tests/assets/infanterie.test.ts` la tient (déterminisme octet par octet, verdicts, lecture par three, conformation dans la tolérance, clips qui bougent les os des trois niveaux, masque binaire, dépôt identique à la production du script). Le temps de ce pilote est celui d'un générateur **maison**, pas d'un générateur externe : **le char léger reste le pilote de cette voie-là** (§3.2), et rien ici ne dit combien coûte un modèle qui vient d'ailleurs. Une chose a cassé au passage, hors du fichier : sous Node, `analyserGlb` ne lisait aucun GLB texturé — `GLTFLoader` lit `self.URL` pour ses images, et `self` n'existe qu'en navigateur —, corrigé dans `render3d/modeles.ts`, avec un test. Deux observations sur le jeu : les quatre cartes sont demandées **une fois par niveau** (trois lectures de fichier, trois demandes, que le cache du navigateur absorbe) ; et la démo passe de 336 à 308 appels de dessin, une escouade coûtant deux mailles skinnées et un socle là où le placeholder en coûtait huit. **Non vérifié** : la beauté, le ton et les interdits, qui restent la relecture humaine de `11-assets-spec.md` §7.3 ; `doc/assets/vitrine-infanterie.jpg` est un document pour cette relecture, pas une preuve.
...
**J1 est acquis** (6 septembre 2026, soir) : ses trois critères sont tenus, et le détail est au §2, « État ». Le lot A a laissé la mesure derrière lui — le budget de triangles est tenu, celui des appels ne l'est pas d'un facteur vingt —, et c'est ce chiffre que J3 devra faire baisser famille par famille.
