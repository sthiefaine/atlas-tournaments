...
| **Maël Orven** — *Le badge gratté* | `rayon_laser` : 5 PV à l'adverse **la plus chère** — le seul super **désigné**, et le seul dont la valeur dépend de ce que l'adversaire a acheté | `infanterie` — ses troupes à pied frappent à 85 % | Gagner **la ligne de nuit** (`aube_nuit_2v2`, là où le Rapace vole) **et trois matchs sans perdre une seule unité**. La Rasante prend la pièce la plus chère du terrain : on gagne son banc en prouvant qu'on n'en perd aucune. |
...
2. **Ariane et Awa avaient la même première phrase** (« tout le monde avance d'une case ») : au menu, il choisissait au hasard. Awa mène désormais par ce qu'elle est la seule à faire — *toute son infanterie rejoue son tour entier*. *(Le 12 septembre 2026, le kit lui-même a suivi : Teranga ne fait plus avancer que les troupes à pied, et les fait frapper — voir `pouvoirs-v4.md`, « Teranga et Rasante ».)*
...
1. **Ariane est un sous-ensemble strict d'Awa, au même prix.** Teranga (3 barres) = L'échappée (3 barres) + les appareils + ×1,15 aux fantassins. Vérifié dans le JSON, c'est exact. **Ce n'est pas un défaut de roster mais un défaut de kit**, et Ariane est verrouillée (tutoriel, mesurée trois fois) : c'est Teranga qu'il faut resserrer — le mouvement **ou** le ×1,15, pas les deux à 3 barres. Porté au §8, hors de mes fichiers. **Tranché le 12 septembre 2026** : les deux effets de Teranga sont filtrés aux troupes à pied (`mouvement: ['pied', 'bottes']`, le filtre de son super), les 3 barres restent. Elle ne contient plus L'échappée — Ariane emmène ce qui roule et ce qui chenille, Awa n'emmène que ce qui marche mais le fait frapper. Mesuré aux deux graines, sur `plaine` et `relief`, dans les deux protocoles : `pouvoirs-v4.md`, « Teranga et Rasante ».
...
4. **Maël à 5 barres est un bouton, pas une récompense** : 8 PV garantis, aucun choix de cible, le super le moins cher de la table, qu'un humain appuiera toutes les cinq barres. Il propose 7 barres, ou laisser le joueur désigner les deux cibles. **Suivi en recommandation** : c'est un chiffre de `commandants-capacites.json`, hors de mes fichiers, et c'est la correction la plus urgente des quatre. **Tranché le 12 septembre 2026, et pas comme il le proposait** : 7 barres seul retire Maël du jeu (moyenne 25,6 % sur seize cellules, mesuré). Rasante devient **6 barres, 5 PV sur l'unité adverse la plus chère, une seule** : la décision n'est pas *qui* mais *quand*, puisque le super ne vaut que ce que l'adversaire a mis sur la table.
...
1. ~~**Teranga contre L'échappée.**~~ **Tranché le 12 septembre 2026** : Teranga est filtré aux troupes à pied, 3 barres inchangées (`pouvoirs-v4.md`, « Teranga et Rasante »).
2. ~~**Maël à 5 barres.**~~ **Tranché le 12 septembre 2026** : Rasante passe à 6 barres et à 5 PV sur la seule unité adverse la plus chère. Reste ouvert, et c'est le chiffre à reprendre : Maël–Tomas sur `relief` monte de 30–55 % à 65–85 %, parce que le pouvoir de Tomas achète exactement les pièces que le rayon chasse.
