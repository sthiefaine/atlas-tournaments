# Opus 1 — Dix tutoriels et dix-huit finales

Plan éditorial du 9 septembre 2026. **Ces fiches ne sont pas des scénarios jouables.** Les quatre premiers tutoriels réutilisent les identifiants, cartes et conditions actuels ; leurs réglages proposés ne sont pas appliqués ici. Les six autres tutoriels et les dix-huit finales restent à cartographier, écrire en scènes, intégrer et tester dans les deux modes.

Le JSON voisin porte les 28 fiches et leurs contrats. Total de l’opus : **10 + 12 × 12 + 18 = 172 missions**. Les quêtes déjà existantes ne s’ajoutent pas silencieusement à ce total : leur reprise ou leur statut d’exhibition devra être identifié dans le manifeste général.

## Cadre de progression

Trois saisons nationales de 48 missions précèdent trois saisons finales de six épisodes. La révélation et le repli tombent en **saison globale 5, épisode 4**, soit finale 10 et mission **164 sur 172**. Il reste huit missions : assez pour faire jouer les conséquences, battre le père loyalement et résoudre le conflit. La fratrie Maël Orven/Lise Varen reste un secret de coulisse pendant tout cet opus. Les noms publics, titres, scènes et biographies joueur ne la révèlent pas. Seul le lien père-fils Edran–Maël est révélé à ce pivot ; aucun lien familial avec Lise ne l’est. Sélène reste la dirigeante et Ost le visage des Gris. L’identité de Relais Zéro reste inconnue après cet opus.

Une mission à repli imposé doit respecter le temps du joueur : briefing initial signalant une opération d’extraction sous pression ; révélation avant la prise de contrôle ; objectif explicite de sortie et récompenses proportionnelles aux groupes sauvés. Aucune invulnérabilité secrète, aucune défaite déclenchée après une victoire acquise. Le résultat stratégique est fixe, les objectifs jouables et leurs conséquences ne le sont pas. Un échec d’extraction permet de recommencer ; une extraction réussie avance la campagne avec la mention « Repli accompli — concession perdue ».

Chaque scénario doit spécifier ses cases, unités objectives, échéances et conditions ET/OU avant intégration. Les descriptions de postes énergétiques utilisent les bâtiments existants ; elles n’inventent pas un système de ressource supplémentaire. Les personnages commandent des équipements de tournoi et aucun objectif ne vise une population ou un réacteur.

## Tutoriels

### T01 — Premier contact (`premier_contact`)

**Apprentissage :** Déplacement, terrain, prévision et riposte. **Victoire :** Mettre hors jeu les trois unités de Tomas.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 10 journées.
**Normal :** Conserver la carte et le contrat existants ; aides contextuelles actives. **Difficile :** Même objectif et unités ; prévision à une journée et aides rappelables.
**État :** existant_a_reviser. Débloquer le tutoriel suivant après victoire.

### T02 — Les quatre villes (`villes_du_bocage`)

**Apprentissage :** Capture, revenus et soins. **Victoire :** Posséder simultanément trois des quatre villes désignées.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 16 journées.
**Normal :** Conserver les quatre villes et les vingt points de capture ordinaires. **Difficile :** Tomas donne priorité aux villes encore libres ; aucune réduction des points de capture.
**État :** existant_a_reviser. Débloquer le tutoriel suivant après victoire.

### T03 — Le chantier des usines (`chantier_des_usines`)

**Apprentissage :** Production, génie et remise en service. **Victoire :** Remettre en service et posséder les deux usines désignées.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 18 journées.
**Normal :** Conserver carte, objectifs et prime du génie ; expliquer le menu de production. **Difficile :** Même économie ; IA protège davantage son infanterie de capture.
**État :** existant_a_reviser. Débloquer le tutoriel suivant après victoire.

### T04 — Le QG de la presqu’île (`qg_de_la_presquile`)

**Apprentissage :** Capture du QG, transport et marées. **Victoire :** Capturer le QG adverse : quarante points.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 18 journées.
**Normal :** Conserver la carte et les marées ; annoncer les quatre tours de capture. **Difficile :** Même fenêtre de marée ; IA intercepte le transport plutôt que recevoir des dégâts gratuits.
**État :** existant_a_reviser. Débloquer le tutoriel suivant après victoire.

### T05 — La bonne distance (`opus1_tutoriel_05`)

**Apprentissage :** Tir indirect et protection. **Victoire :** Capturer le QG du plateau ou mettre hors jeu toute la défense.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 14 journées.
**Normal :** Deux couloirs permettent de déplacer puis installer l’artillerie ; pas de tir après déplacement. **Difficile :** Une reconnaissance adverse cherche le flanc ; défense et dégâts inchangés.
**État :** conception_non_jouable. Débloquer le tutoriel suivant après victoire.

### T06 — Au-delà des arbres (`opus1_tutoriel_06`)

**Apprentissage :** Brouillard, radar et drones. **Victoire :** Capturer le radar central puis le QG adverse ; la prise du radar reste acquise dans le journal d’objectif.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 16 journées.
**Normal :** Drone observateur et radar servent à préparer un combat réel. **Difficile :** Brouilleur adverse annoncé dans le briefing ; information jamais omnisciente.
**État :** conception_non_jouable. Débloquer le tutoriel suivant après victoire.

### T07 — Le dernier kilomètre (`opus1_tutoriel_07`)

**Apprentissage :** Munitions, ravitaillement et transport. **Victoire :** Faire parvenir le transport désigné à la sortie nord en conservant au moins une unité de couverture.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 14 journées. Transport désigné hors jeu.
**Normal :** Ravitailleur et transport fournis ; un détour protège le convoi. **Difficile :** Une patrouille tente de couper le détour ; réserve de munitions identique.
**État :** conception_non_jouable. Débloquer le tutoriel suivant après victoire.

### T08 — Une ligne commune (`opus1_tutoriel_08`)

**Apprentissage :** Équipes, vision partagée et caisses séparées. **Victoire :** En équipe 2v1, capturer le QG adverse ou mettre hors jeu son armée.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 16 journées.
**Normal :** Allié annonce sa route ; enseigner le ravitaillement allié sans transfert de propriété. **Difficile :** Allié autonome moins directif ; adversaire concentre ses attaques sur les positions découvertes.
**État :** conception_non_jouable. Débloquer le tutoriel suivant après victoire.

### T09 — Le prix du courant (`opus1_tutoriel_09`)

**Apprentissage :** Énergie et concessions. **Victoire :** Capturer deux des trois postes de distribution représentés par des bâtiments existants.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 16 journées.
**Normal :** Les revenus viennent des bâtiments ; aucune nouvelle ressource de combat implicite. **Difficile :** Deux postes plus disputés ; même revenu et même coût de production.
**État :** conception_non_jouable. Débloquer le tutoriel suivant après victoire.

### T10 — La première promesse (`opus1_tutoriel_10`)

**Apprentissage :** Synthèse et choix persistant. **Victoire :** Capturer le QG adverse ou mettre hors jeu son armée ; protéger le transport est un objectif secondaire facultatif.
**Défaite :** Perdre son QG lorsque la carte en comporte un. Toutes les unités du joueur hors jeu. Dépasser la limite de 18 journées.
**Normal :** Tous les rappels restent consultables ; choisir ensuite un contrat de maintenance ou des fonds immédiats. **Difficile :** IA coordonnée, même objectif ; le choix narratif garde exactement la même portée.
**État :** conception_non_jouable. Ouvrir la première saison nationale.

Choix final : maintenance partagée (ravitailleur) ou fonds immédiats (+1 000) dans la première mission nationale ; conséquence figée au départ.

## Finales de la Cinquième Manche

### F01 — Les portes du réseau · saison 4, épisode 1

**Adversaire :** `cmd_hadran_ost` ; **format :** 2v1. Port fluvial ; deux ponts distants, ateliers centraux.
**Objectif :** Capturer le QG d’Ost ou mettre hors jeu ses unités.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Les Gris assument leur rattachement à la Cinquième Manche.
**Choix :** Ouvrir le port aux délégations hésitantes ou réserver les quais à la coalition. **Conséquence :** Un quai allié disponible en finale 5 ou 1 500 fonds en finale 2.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F02 — La taxe des sommets · saison 4, épisode 2

**Adversaire :** `cmd_basile_kelm` ; **format :** 1v1. Trois crêtes, chemins de traverse, radar exposé.
**Objectif :** Capturer deux des trois postes du col.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Basile applique un contrat de priorité qui coupe les petites délégations.
**Choix :** Maintenir les relais publics ou emporter les batteries mobiles. **Conséquence :** Radar allié initial en finale 6 ou transport supplémentaire en finale 4.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F03 — Le ciel verrouillé · saison 4, épisode 3

**Adversaire :** `cmd_mael_orven` ; **format :** 1v2. Plaine aéroportuaire, hangars bas et haies.
**Objectif :** Capturer les deux QG adverses ou mettre hors jeu leurs deux armées.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Maël Orven impose son rythme aérien ; ses liens familiaux ne sont pas connus du joueur.
**Choix :** Échanger les journaux de vol ou protéger leur source. **Conséquence :** Position d’un renfort révélée en finale 9 ou confiance accrue d’une délégation hésitante.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F04 — Le contrat de Lise · saison 4, épisode 4

**Adversaire :** `cmd_lise_orven` ; **format :** 2v2. Canal industriel, deux lignes indirectes et îlots.
**Objectif :** Capturer les QG de la coalition adverse ou mettre hors jeu ses armées.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Lise défend la centralisation pour prévenir les coupures ; elle critique déjà les méthodes de Maël.
**Choix :** Garantir l’accès public demandé par Lise ou exiger son retrait immédiat. **Conséquence :** Ouvre une médiation en finale 13 ou réduit la défense de finale 5.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F05 — Les pavillons hésitants · saison 4, épisode 5

**Adversaire :** `cmd_yuna_serrat` ; **format :** 2v2. Ville basse avec itinéraire civil hors plateau de combat.
**Objectif :** Capturer les QG adverses ou mettre hors jeu leurs armées.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Une délégation ayant choisi un contrat adverse peut combattre ici ; ce ne sont pas tous ses habitants qui ont changé de camp.
**Choix :** Offrir une audition à la délégation battue ou suspendre son contrat. **Conséquence :** Renfort conditionnel en finale 11 ou fonds de compensation en finale 6.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F06 — Le signal sans visage · saison 4, épisode 6

**Adversaire :** `cmd_sans_visage` ; **format :** 1v3. Archipel de pylônes, pontons et zones boisées.
**Objectif :** Capturer trois postes de transmission ou mettre hors jeu les trois armées adverses.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Une voix inconnue coordonne les adversaires ; aucune identité ni parenté n’est donnée.
**Choix :** Publier les relais compromis ou surveiller leur trafic. **Conséquence :** Brouillage adverse réduit en finale 16 ou itinéraire révélé en finale 8.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F07 — Les réserves verrouillées · saison 5, épisode 1

**Adversaire :** `cmd_basile_kelm` ; **format :** 3v1. Trois entrées vers un bassin de stockage.
**Objectif :** Capturer le QG de Basile ou mettre hors jeu sa défense.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Les réserves servent à imposer la tutelle sur Aube.
**Choix :** Répartir les réserves ou équiper la colonne de tête. **Conséquence :** Ravitaillement allié en finale 12 ou unité lourde initiale en finale 8.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F08 — La contre-offre · saison 5, épisode 2

**Adversaire :** `selene_veyr` ; **format :** 1v2. Deux boucles routières et un poste central contesté.
**Objectif :** Posséder deux postes de distribution à la fin de J18 ou capturer les deux QG adverses avant cette échéance.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Sélène propose de garantir l’énergie en échange de l’exclusivité des décisions.
**Choix :** Rendre l’offre publique ou négocier un délai transparent sous contrôle de la coalition. **Conséquence :** Confiance des alliés en finale 17 ou arrivée adverse retardée de deux journées en finale 9.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles. Objectif composé ou contrôle évalué à une échéance à implémenter.

### F09 — Le dépôt de relève · saison 5, épisode 3

**Adversaire :** `cmd_mael_orven` ; **format :** 2v1. Vallée resserrée, dépôt de relève au fond.
**Objectif :** Capturer le QG de Maël ou mettre hors jeu sa force avancée.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Edran Sorel apparaît comme un logisticien influent ; la relation paternelle reste non confirmée.
**Choix :** Évacuer les techniciens du dépôt ou sécuriser les archives logistiques. **Conséquence :** Une équipe de secours supplémentaire en finale 10 ou preuve utilisable à la médiation de finale 13.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F10 — Ce que Sorel protège · saison 5, épisode 4

**Adversaire :** `cmd_edran_sorel` ; **format :** 1v2. Bassin du dépôt ; trois sorties secondaires pour les colonnes sauvées.
**Objectif :** Repli scénarisé : faire sortir le commandement désigné par la porte sud avant la fin de J8. La concession est perdue dans tous les cas.
**Défaite / transition :** Commandement de repli hors jeu ou non sorti à la fin de J8 : recommencer la mission. Défaite stratégique imposée, transition réussie après extraction ; pas de victoire factice ni trophée de conquête. Le journal distingue repli accompli et concession perdue.
**Normal :** J3 : arrivée annoncée d’Edran par une zone extérieure interdite au déploiement initial ; forces finies. Trois couloirs de sortie, huit journées pour le commandement. **Difficile :** Même repli obligatoire et huit journées ; une voie secondaire est plus disputée, sans fermer la sortie principale ni rendre le commandement ciblable avant sa fenêtre de décision.
**Révélation :** Edran révèle être le père de Maël, rejoint son fils et bat notre coalition. La révélation précède les ordres, pas le résultat.
**Choix :** Sauver les réserves, les archives ou le matériel d’une délégation isolée ; objectifs secondaires incompatibles faute de temps. **Conséquence :** Bonus distincts en finales 11–13 ; le retrait stratégique est fixé, les moyens sauvés et les relations restent ceux du joueur.
**À développer :** Résultat de campagne repli accompli distinct du résultat de match. Extraction de plusieurs groupes et journal des groupes sauvés. Entrée scénarisée d’un commandant avec scène bloquant les tours IA. Issue stratégique fixe annoncée explicitement après révélation ; conservation des objectifs secondaires.

### F11 — Les comptes du repli · saison 5, épisode 5

**Adversaire :** `cmd_yuna_serrat` ; **format :** 2v1. Ravines et route de desserte, poursuite bornée.
**Objectif :** Conduire le transport du commandement à la sortie et conserver au moins une unité alliée.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Transport désigné hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Les délégations débattent de la défaite ; les fautes du commandement ne sont pas effacées.
**Choix :** Admettre publiquement l’échec ou demander une enquête contradictoire avant publication. **Conséquence :** Dialogue et soutien de finale 14 ; aucune option ne réécrit la perte de finale 10.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles. Objectif composé ou contrôle évalué à une échéance à implémenter.

### F12 — Les lignes qui restent · saison 5, épisode 6

**Adversaire :** `cmd_basile_kelm` ; **format :** 2v2. Deux lignes de retenue, barrage et second gué.
**Objectif :** Tenir jusqu’à la fin de J20 ou capturer les deux QG adverses.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Basile veut empêcher la reconstitution de la coalition.
**Choix :** Garantir les équipes qui refusent la tutelle ou concentrer la défense sur le dépôt. **Conséquence :** Allié supplémentaire en finale 17 ou réserve initiale en finale 14.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F13 — Le contrat de Varen · saison 6, épisode 1

**Adversaire :** `cmd_lise_orven` ; **format :** 1v1. Terrasses, contre-pentes et deux couloirs d’artillerie.
**Objectif :** Capturer le QG de Lise ou mettre hors jeu sa défense.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Lise confronte les garanties proposées aux actes des dernières saisons ; son engagement personnel ne dépend pas d’une parenté révélée.
**Choix :** Proposer une reddition contractuelle contrôlée ou imposer sa suspension après victoire. **Conséquence :** Médiation crédible : elle peut aider finale 17 ; promesses rompues : elle refuse et reste adverse. Sa conversion n’est jamais automatique.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F14 — Le fils au premier rang · saison 6, épisode 2

**Adversaire :** `cmd_mael_orven` ; **format :** 1v2. Double aérodrome, axe rapide vulnérable au brouillage.
**Objectif :** Capturer les deux QG adverses ou mettre hors jeu les deux armées.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Maël refuse de réduire son ambition au projet de son père.
**Choix :** Préserver un accès aux équipes dissidentes ou saisir leurs réserves. **Conséquence :** Évacuation facilitée en finale 15 ou ravitaillement initial supplémentaire en finale 16.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F15 — La dette de Sorel · saison 6, épisode 3

**Adversaire :** `cmd_edran_sorel` ; **format :** 2v1. Dépôts en escalier et circulation à sens contraint.
**Objectif :** Capturer le QG d’Edran ou mettre hors jeu sa colonne.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Edran justifie son ralliement sans nier sa responsabilité ; le joueur peut le battre loyalement cette fois.
**Choix :** Accepter sa déposition sous contrôle ou refuser toute participation opérationnelle. **Conséquence :** Des preuves supplémentaires pour l’épilogue ou un contrôle public renforcé ; il ne remplace pas le joueur et ne décide pas du sort des autres commandants.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F16 — Le réseau muet · saison 6, épisode 4

**Adversaire :** `cmd_sans_visage` ; **format :** 1v3. Trois îles techniques ; relais visibles, drones en réserve.
**Objectif :** Capturer les trois postes de transmission ou mettre hors jeu toutes les forces adverses.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** L’inconnu perd ses relais mais son identité demeure inconnue à la fin de l’opus.
**Choix :** Conserver une trace sous surveillance ou isoler immédiatement le dernier relais. **Conséquence :** Piste narrative du prochain opus ou garanties renforcées dans l’épilogue ; aucun combat supplémentaire imposé.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F17 — Les droits d’Aube · saison 6, épisode 5

**Adversaire :** `cmd_hadran_ost` ; **format :** 3v1. Plateau expérimental fictif ; trois accès, aucun réacteur à détruire.
**Objectif :** Capturer le QG d’Ost ou mettre hors jeu son dispositif.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre la finale suivante.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** Ost perd la maîtrise du terrain ; les alliés présents dépendent des engagements et médiations.
**Choix :** Privilégier l’accès partagé ou exiger un contrôle de coalition temporaire. **Conséquence :** Modifie le traité final et les dialogues de finale 18 ; ni énergie infinie ni mise en service magique.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

### F18 — La dernière concession · saison 6, épisode 6

**Adversaire :** `selene_veyr` ; **format :** 2v2. Campus de coordination, jardins techniques et deux axes de capture.
**Objectif :** Capturer les QG de la coalition de Sélène ou mettre hors jeu toutes ses unités.
**Défaite / transition :** QG du joueur perdu si présent. Toutes les unités du joueur hors jeu. Victoire ouvre l’épilogue et conserve le journal pour le prochain opus.
**Normal :** Forces bornées, intentions et renforts annoncés dans le briefing. Tester capture et élimination quand les deux voies sont prévues. **Difficile :** IA protège les indirects et coordonne les captures ; placements et patrouilles différents annoncés avant lancement. Aucun coefficient de dégâts secret.
**Révélation :** La direction de Sélène est confirmée et contestée publiquement ; son identité n’était pas le mystère familial.
**Choix :** Signer le réseau partagé, un mandat de transition ou refuser un traité insuffisant selon les engagements accumulés. **Conséquence :** Épilogue calculé sur droits, confiance, preuves et délégations ralliées ; l’inconnu reste une ouverture sans annuler la victoire.
**À développer :** Liaison au journal de campagne et aux alliances conditionnelles.

## Réception avant publication

Vérifier schémas, création de partie, absence de commandes IA illégales et au moins une issue réalisable dans chacun des modes. Puis jouer humainement : durée, tours sans décision, lisibilité des renforts, compréhension des objectifs et conséquences. Pour F10, tester séparément la sortie minimale et chaque sauvetage ; interdire une graine sans solution d’extraction. Les numéros de mission ne constituent jamais une preuve de jouabilité.

Limites éditoriales : finales ordinaires J24 ; F8 J18, F10 J8, F11 J16, F12 J20. Ces limites sont des hypothèses de conception à ajuster après simulation et jeu humain, pas des durées annoncées comme équilibrées. Le JSON précise les échecs à échéance et les liens vers la mission suivante.

## Extension — technologie méridienne annoncée

Le drone marin, les stations IEM et la météo de simulation enrichissent les missions existantes sans changer les **172 missions ni les huit adversaires**. Les fichiers JSON identifient précisément les fiches concernées. Ces ajouts restent des intentions éditoriales tant que leurs cartes et paramètres ne sont pas validés.

La supériorité adverse vient du matériel et du placement. Les stations ont une position, une portée et un calendrier consultables avant engagement. Le joueur peut capturer la source, rejoindre un refuge ou se replier sur une route moins pénalisée. Le mode difficile renforce la défense et les patrouilles, jamais une portée ou activation cachée. Les nations adaptent leurs paysages et leurs stratégies sans immunités nationales implicites. Le forçage météo concerne le terrain de tournoi, pas le climat de populations réelles.

Aucune scène publique nouvelle ne révèle la fratrie ; les doctrines techniques ne fournissent aucun indice familial imposé. La révélation père-fils reste au pivot prévu, sans révéler le lien avec Lise.

Limite du premier essai : la capture interrompt la menace IEM adverse. La météo reste programmée ; sa commande par station capturable est une proposition ultérieure. Toute adaptation de camp est explicite dans le scénario et le briefing, jamais déduite secrètement du pays.
