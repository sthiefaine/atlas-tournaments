...
- **La mesure** (même révision) : `Rendu.mesurer()` rend `{ triangles, appels, msParImage, composeur }` depuis `renderer.info`, dont la remise à zéro automatique est coupée pour compter **l'image entière** — passe d'ombres comprise et, quand la chaîne est active, la seconde passe de scène (normales et profondeur) et les passes plein écran, c'est-à-dire le coût réel. `window.__atlas.mesurer()` en jeu ; `window.__atlasBanc.mesurer()` et `qualite(v)` sur le banc, dont la section « Rendu » affiche le relevé une fois par seconde. C'est l'instrument de `16-realisme.md` A6 : la table du §9.2 attend toujours ses chiffres.
